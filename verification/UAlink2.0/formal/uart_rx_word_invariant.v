...
module uart_rx_word_invariant #( // 单个实际账户完整字顺序关系检查模块。
    ...
    wire [C_DEPTH*C_WORD_WIDTH-1:0] aligned_words; // 将独立顺序队列一次对齐全部真实逻辑行。
    genvar gen_rotation; // 每一级按固定字数旋转整个不透明队列。
    generate // 只对推荐一百二十八字容量使用整体循环对齐。
        if (C_DEPTH == 128) begin : gen_align_default // 独立定理证明旋转索引与原有效行索引一致。
            wire [6:0] rotation; // 队列预取前缀减去实际读地址的模一百二十八差。
            wire [8*C_DEPTH*C_WORD_WIDTH-1:0] stages; // 七级固定分块交换保持所有完整字位。
            assign rotation = prefetched[6:0] - read_addr[6:0]; // 原数量关系保证每个有效索引都在范围内。
            assign stages[0 +: C_DEPTH*C_WORD_WIDTH] = shadow_words; // 起点为原输入构造的独立顺序队列。
            for (gen_rotation=32'd0; gen_rotation<32'd7; gen_rotation=gen_rotation+32'd1) begin : gen_rotate_stage // 逐位构成固定字距的整体循环右移。
                localparam integer C_SHIFT=(32'd1<<gen_rotation)*C_WORD_WIDTH; // 当前级只交换固定长度的完整字片段。
                wire [C_DEPTH*C_WORD_WIDTH-1:0] previous; // 当前级原完整队列字序。
                assign previous=stages[gen_rotation*C_DEPTH*C_WORD_WIDTH +: C_DEPTH*C_WORD_WIDTH]; // 前级实际组合结果保持完整位宽。
                assign stages[(gen_rotation+1)*C_DEPTH*C_WORD_WIDTH +: C_DEPTH*C_WORD_WIDTH]=rotation[gen_rotation]?{previous[C_SHIFT-1:0],previous[C_DEPTH*C_WORD_WIDTH-1:C_SHIFT]}:previous; // 固定片段拼接避免每一行建立独立宽选择器。
            end // 结束当前固定距离旋转级。
            assign aligned_words=stages[7*C_DEPTH*C_WORD_WIDTH +: C_DEPTH*C_WORD_WIDTH]; // 每个固定物理行对应其原输入期望字。
        end else begin : gen_align_unused // 其它容量仍使用原逐行索引表达式。
            assign aligned_words={C_DEPTH*C_WORD_WIDTH{1'b0}}; // 未选中的观察锥不影响任何内容性质。
        end // 结束推荐容量整体对齐选择。
    endgenerate // 结束独立队列整体旋转生成块。
    ...
    generate // 固定逐字展开，实际存储与shadow布局不同。
        for (gen_word = 32'd0; gen_word < C_WORDS; gen_word = gen_word+32'd1) begin : gen_words // 当前逻辑地址与顺序位置。
            ...
            if (C_DEPTH == 128) begin : gen_aligned_row // 每一有效实际行仍参加完整字检查。
                assign bad_memory[gen_word]=(offset<unread)&&(i_memory[gen_word*C_WORD_WIDTH +: C_WORD_WIDTH]!=aligned_words[gen_word*C_WORD_WIDTH +: C_WORD_WIDTH]); // 原有效行对应独立队列整体对齐后的固定位置。
            end else begin : gen_original_row // 非默认容量保持原始索引语义。
...
endmodule // 结束uart_rx_word_invariant只读内容关系。
