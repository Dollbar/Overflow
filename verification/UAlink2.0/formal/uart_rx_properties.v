...
module uart_rx_properties #(parameter integer C_DEPTH=128, parameter integer C_CONTENT=0) ( // UART 接收独立状态和可选实际内容检查模块。
    ...
    generate // 内容模式检查实际 SRAM 行和注册 Q 与原输入顺序队列而无数据假设。
        if (C_CONTENT!=0) begin : gen_content // 内容模式检查实际 SRAM 行和注册 Q 与原输入顺序队列而无数据假设。
            upli_receive_word_invariant #(.C_DEPTH(C_DEPTH),.C_WORD_WIDTH(32),.C_COUNT_WIDTH(12)) Content_Inst ( // 内容模式检查实际 SRAM 行和注册 Q 与原输入顺序队列而无数据假设。
                .i_clk(i_clk),.i_rstn(running),.i_push(expected_payload_write),.i_pop(consume),.i_word(i_word), // 内容模式检查实际 SRAM 行和注册 Q 与原输入顺序队列而无数据假设。
                ...
                .i_pending(actual_pending),.i_cached(actual_cached),.i_head(actual_head),.i_tail(actual_tail), // 内容模式检查实际 SRAM 行和注册 Q 与原输入顺序队列而无数据假设。
                .i_read_result(actual_read_result),.i_memory(actual_memory),.o_shadow_head(shadow_head),.o_violation(bad_content) // 内容模式检查实际 SRAM 行和注册 Q 与原输入顺序队列而无数据假设。
            ); // 内容模式检查实际 SRAM 行和注册 Q 与原输入顺序队列而无数据假设。
        end else begin : gen_control // 内容模式检查实际 SRAM 行和注册 Q 与原输入顺序队列而无数据假设。
            assign shadow_head=32'd0; // 内容模式检查实际 SRAM 行和注册 Q 与原输入顺序队列而无数据假设。
