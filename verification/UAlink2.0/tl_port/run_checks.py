...
R=(lambda _ualink_file: next(((_ualink_dir / (_ualink_dir / '.ualink-root').read_text(encoding='utf-8').strip()).resolve() for _ualink_dir in _ualink_file.parents if (_ualink_dir / '.ualink-root').is_file()), Path(__file__).resolve().parents[2]))(__import__('pathlib').Path(__file__).resolve());D=R;S=R/'build/verification/tl_port';S.mkdir(parents=True,exist_ok=True);sources=[p for p in sorted((R/'rtl/tl').glob('*.v')) if p.name not in ('tl_credit_publish.v','tl_receive_storage.v','tl_receive_context.v')];rows=[]
for w in (8,16):
