...
R=(lambda _ualink_file: next(((_ualink_dir / (_ualink_dir / '.ualink-root').read_text(encoding='utf-8').strip()).resolve() for _ualink_dir in _ualink_file.parents if (_ualink_dir / '.ualink-root').is_file()), Path(__file__).resolve().parents[2]))(__import__('pathlib').Path(__file__).resolve());p=argparse.ArgumentParser(description=__doc__);p.add_argument('--kd28-root',type=Path,required=True);p.add_argument('--label',default='checks');p.add_argument('--peers-label',default='peers_semantics');p.add_argument('--replace',type=Path);p.add_argument('--dependency-root',type=Path);p.add_argument('--boundary-fault',choices=('missing_guard','truncated_header'),default='missing_guard');a=p.parse_args();S=R/'build/verification/tl_control_partition'
