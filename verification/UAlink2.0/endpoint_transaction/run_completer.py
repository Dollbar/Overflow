#!/usr/bin/env python3
"""Exercise the real one-to-four-slot read Completer with an independent memory BFM.
...
localparam CAPACITY=@@CAPACITY@@,SLOT_WIDTH=@@SLOT_WIDTH@@; // 显式记录本轮真实容量和接口位宽。
...
reg [1:0] vc=0,asi=0,accepted=0; reg [SLOT_WIDTH-1:0] result_slot=0; // 区分固定字段宽度与本轮slot位宽。
reg pool=0; reg [511:0] result_data=0; reg [3:0] result_status=0; // 内存返回数据只能由BFM驱动。
wire rr,mvalid,mrready,sv,error; wire [1:0] dvalid; wire [SLOT_WIDTH-1:0] mslot; // 观察请求、内存和发送所有权。
...
integer pending[0:CAPACITY-1],due[0:CAPACITY-1],slot_for[0:23]; // BFM只为真实容量保留任务。
...
reg [511:0] held_mem; reg [SLOT_WIDTH-1:0] held_slot; reg mem_stalled=0; // 检查内存请求反压稳定性。
endpoint_read_completer @@DUT_PARAMETERS@@ dut(.i_clk(clk),.i_rstn(rstn),.i_local_id(local_id), // 自动位宽用DUT默认参数，显式位宽则覆盖。
 ...
 if(rstn) begin // 正常周期检查明确期望的错误分类。
  ...
  if(tracking) begin // 正常事务阶段记录独立请求到响应完整链条。
   ...
   if(mvalid && mready) begin // 内存BFM只能由实际命令握手创建返回任务。
    if(mslot>=CAPACITY) $fatal(1,"FAIL memory_slot_range slot=%0d capacity=%0d",mslot,CAPACITY); // 不允许指针访问补齐的虚构槽。
   ...
   if(rv && !rr && count==CAPACITY) full_stalls=full_stalls+1; // 明确观察实际容量满后的背压。
  end // 结束正常流量参考检查。
 end // 结束非复位检查。
...
begin // 开始一次正常事务批次。
 ...
 for(k=0;k<CAPACITY;k=k+1) pending[k]=-1; // 所有真实BFM slot开始为空。
 ...
 for(loop_count=0;loop_count<1200 && retired<batch_limit;loop_count=loop_count+1) begin // 设置有限上界拒绝永久阻塞。
  ...
  for(k=CAPACITY-1;k>=0;k=k-1) if(pending[k]>=0 && due[k]<=cycles && chosen<0) chosen=k; // 优先高slot以激励真实乱序完成。
  if(loop_count==6) begin mrv=1;result_slot=0;result_status=0;return_kind=2;expecting_error=1; end // 四槽已分配但未执行时注入非法完成。
  else if(loop_count>=7 && loop_count<7+(1<<SLOT_WIDTH)-CAPACITY) begin mrv=1;result_slot=CAPACITY+loop_count-7;result_status=0;return_kind=2;expecting_error=1; end // 满槽时逐个拒绝全部可编码的未用slot，不能别名更新有效槽。
...
initial begin // 准備明确的请求向量和字节内存，先测非法因果事件。
 ...
 for(p=CAPACITY;p<(1<<SLOT_WIDTH);p=p+1) begin result_slot=p;step();end // 空表逐个验证未用slot返回被诊断。
 ...
 if(total_requests!=32 || total_memory!=32 || total_results!=32 || total_responses!=32 || partial==0 || double_accept==0 || full_stalls==0 || high_addresses==0 || (CAPACITY>1 && ooo==0) || header_first==0 || data_first==0 || memory_stalls==0) $fatal(1,"FAIL coverage"); // 单槽无法乱序，其余容量要求真实乱序样本。
...
def main() -> int:
    ...
    parser.add_argument("--capacity", type=int, choices=(1, 2, 3, 4), default=4)
    parser.add_argument("--slot-width", type=int, choices=(1, 2), help="omit to exercise the DUT automatic width")
    parser.add_argument("--normal-only", action="store_true", help="skip the isolated RTL mutations")
    parser.add_argument("--synth", action="store_true", help="also check Verilog-2001 and mapped generic synthesis")
    args = parser.parse_args()
    slot_width = args.slot_width or max(1, (args.capacity - 1).bit_length())
    if (1 << slot_width) < args.capacity: parser.error("slot width cannot represent every real slot")
    ...
    parameters = f"#(.CAPACITY({args.capacity})" + (f",.SLOT_WIDTH({slot_width})" if args.slot_width else "") + ")"
    tb = TB.replace("@@CAPACITY@@", str(args.capacity)).replace("@@SLOT_WIDTH@@", str(slot_width)).replace("@@DUT_PARAMETERS@@", parameters)
    ...
    summary = {"scope": "single64B ordinary Read Completer, not full Endpoint", "mode": args.mode,
               "capacity": args.capacity, "slot_width": slot_width, "slot_width_mode": "explicit" if args.slot_width else "automatic",
               "source_sha256": hashlib.sha256(RTL.read_bytes()).hexdigest(), "encoder_sha256": hashlib.sha256(ENCODER.read_bytes()).hexdigest(), "cases": []}
    mutations = {
        "address_msb": ("assign o_mem_address = issue_address;", "assign o_mem_address = {1'b0,issue_address[55:0]};"),
        "early_response": ("busy_q[head_q] && complete_q[head_q]", "busy_q[head_q] && issued_q[head_q]"),
        "upper_data": ("head_data[511:256]", "(head_data[511:256] ^ 256'h1)"),
    }
    if args.capacity != (1 << slot_width):
        mutations["slot_wrap"] = ("==CAPACITY-1", "==((1<<SLOT_WIDTH)-1)")
    cases = [None] if args.mode == "shell" or args.normal_only else [None, *mutations]
    for mutation in cases:
        ...
        if record["compile"]["returncode"] == 0:
            ...
            if mutation:
                record["status"] = "detected" if code is not None and code > 0 and re.search(r"FAIL (memory_descriptor|memory_slot_range|response_before_memory_result|data_low|data_high|slot_lifetime|batch_drain)", log) and "PASS" not in log else "mutation_missed"
            else:
    ...
    if args.synth:
        ...
        synthesis.mkdir()
        wrapper = "module capacity_check; endpoint_read_completer " + parameters + " dut(); endmodule\n"
        (synthesis / "wrapper.v").write_text(wrapper)
        summary["verilog2001"] = execute([compiler, "-g2001", "-s", "capacity_check", "-o", "syntax.vvp", "wrapper.v", str(output / "source.v"), str(output / ENCODER.name)], synthesis, "verilog2001")
        yosys = shutil.which("yosys")
        if not yosys: parser.error("--synth requires yosys")
        commands = ["read_verilog ../source.v ../endpoint_response_encode.v", f"chparam -set CAPACITY {args.capacity}" + (f" -set SLOT_WIDTH {slot_width}" if args.slot_width else "") + " endpoint_read_completer", "hierarchy -check -top endpoint_read_completer", "proc", "opt", "memory", "opt", "check -assert", "stat", "write_json netlist.json"]
        (synthesis / "run.ys").write_text("\n".join(commands) + "\n")
        summary["synthesis"] = execute([yosys, "-Q", "-T", "-s", "run.ys"], synthesis, "synthesis")
    checks = all(c["status"] in ("pass", "detected") for c in summary["cases"])
    summary["status"] = "pass" if checks and all(summary[k]["returncode"] == 0 for k in ("verilog2001", "synthesis") if k in summary) else "fail"
