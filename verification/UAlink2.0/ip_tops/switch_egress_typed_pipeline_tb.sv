...
wire [P-1:0] capture_ready=dut.pipeline_ready;
assign valid=dut.pipeline_valid;assign data=dut.pipeline_data;assign last=dut.pipeline_last;
assign token=dut.pipeline_token;assign out_vc=dut.pipeline_vc;assign kind=dut.pipeline_response;
...
switch_egress_typed_pipeline #(.PORTS(P),.VCS(V),.TOKEN_WIDTH(T),.UNIT_WIDTH(U)) dut(
...
.o_valid(typed_valid),.o_data(typed_data),.o_last(typed_last),.o_token(typed_token),.o_vc(typed_vc),.o_response(typed_kind),.o_local_dl_header(typed_header),.o_record_aux(typed_aux),.o_tl_msg(typed_msg),.o_tl_flit(typed_flit),.o_repack_input_error(repack_error),.o_selected(selected),.o_owned(owned),
...
initial begin
 ...
 $display("TYPED_PIPELINE_PASS ports=%0d vcs=%0d cycles=%0d admissions=%0d completed=%0d releases=%0d words=%0d stalls=%0d reset_cancel=1",P,V,cycles,admissions,finishes,releases,words,stall_edges);$finish;
end
endmodule
