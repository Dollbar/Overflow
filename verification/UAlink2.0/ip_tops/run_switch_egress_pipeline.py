...
Artifacts: ROOT/build/verification/ip_tops/switch_egress_pipeline/NEW.
...
def main():
 p=argparse.ArgumentParser(description=__doc__);p.add_argument('--label',required=True);p.add_argument('--rtl',type=Path,default=ROOT/'rtl/switch/switch_egress_pipeline.v');p.add_argument('--scheduler',type=Path,default=ROOT/'rtl/switch/switch_egress_scheduler.v');p.add_argument('--project-root',type=Path,default=ROOT);p.add_argument('--faults',action='store_true');a=p.parse_args()
 if not re.fullmatch('[A-Za-z0-9_-]+',a.label):p.error('fresh safe label')
 stage=ROOT/'build/verification/ip_tops/switch_egress_pipeline'/a.label;stage.mkdir(parents=True,exist_ok=False);source=stage/'source';source.mkdir();hashes={};files=[]
 paths=[a.rtl.resolve(),a.scheduler.resolve()]+[a.project_root.resolve()/'rtl/switch'/n for n in ['switch_egress_vc_queues.v','switch_egress_packet_queue.v','switch_credit_reservation.v']]+[a.project_root.resolve()/'rtl/upli/upli_receive_fifo.v']
 for f in paths+[Path(__file__),HERE/'switch_egress_pipeline_tb.sv']:
 ...
 rtl=source/'switch_egress_pipeline.v';results=[]
 ...
 for ports in [1,2,4]:
  for vcs in [1,2,4]:
   for fault in [None]+(['ready','owner','class','vc','last'] if ports==4 and vcs==4 and a.faults else []):
    folder=stage/f'p{ports}_v{vcs}{"_"+fault if fault else ""}';folder.mkdir();actual=files
    if fault:
     old,new={'ready':('.i_ready(queue_ready)',".i_ready({(2*VCS*PORTS){1'b1}})"),'owner':('.i_body_token(i_body_token)', '.i_body_token(i_header_token)'), 'class':('.o_response(o_response)', '.o_response()'), 'vc':('.o_vc(o_vc)', '.o_vc()'), 'last':('.o_last(o_last)', '.o_last()')}[fault]
     text=rtl.read_text()
     if text.count(old)!=1:raise ValueError('fault anchor '+fault)
     text=text.replace(old,new)
     if fault in ['class','vc','last']:
      text=text.replace('endmodule',{'class':"assign o_response={PORTS{1'b0}};\n",'vc':"assign o_vc={(PORTS*2){1'b0}};\n",'last':"assign o_last={PORTS{1'b0}};\n"}[fault]+'endmodule')
     altered=folder/rtl.name;altered.write_text(text);actual=[altered if f==rtl else f for f in files]
    cmd={'compile':run(folder,'compile',['iverilog','-g2012','-s','switch_egress_pipeline_tb',f'-Pswitch_egress_pipeline_tb.P={ports}',f'-Pswitch_egress_pipeline_tb.V={vcs}','-o','sim.vvp',source/'switch_egress_pipeline_tb.sv',*actual])}
    if cmd['compile']['returncode']==0:
     cmd['simulate']=run(folder,'simulate',['vvp','sim.vvp'])
     if not fault:
      top='switch_egress_pipeline';cmd['g2001']=run(folder,'g2001',['iverilog','-g2001','-s',top,f'-P{top}.PORTS={ports}',f'-P{top}.VCS={vcs}','-o','elab.vvp',*actual]);cmd['lint']=run(folder,'lint',['verilator','--lint-only','--language','1364-2001','-Wall','--top-module',top,f'-GPORTS={ports}',f'-GVCS={vcs}',*actual]);cmd['yosys']=run(folder,'yosys',['yosys','-Q','-T','-p','read_verilog '+' '.join(map(str,actual))+f'; chparam -set PORTS {ports} -set VCS {vcs} {top}; hierarchy -check -top {top}; proc; opt; check -assert; stat; write_json hierarchy.json'])
    log=(folder/'simulate.log').read_text() if (folder/'simulate.log').exists() else ''
    passed=all(c['returncode']==0 for c in cmd.values()) and 'PIPELINE_PASS' in log
    if fault:
     expected={'ready':['PIPELINE_RELEASE_ONCE','PIPELINE_STORAGE_ACCOUNT','PIPELINE_RESERVATION'],'owner':['PIPELINE_UNEXPECTED_ERROR'],'class':['PIPELINE_CLASS_VC_TOKEN'],'vc':['PIPELINE_CLASS_VC_TOKEN'],'last':['PIPELINE_PAYLOAD_LAST']}[fault]
    ...
    if not fault and (folder/'hierarchy.json').exists():
     ...
     visit('switch_egress_pipeline');latches=sum('latch' in cell['type'].lower() for m in modules.values() for cell in m.get('cells',{}).values());structure={'instances':counts,'latches':latches}
     passed=passed and latches==0 and all(counts.get(k)==v for k,v in {'switch_egress_vc_queues':1,'switch_egress_scheduler':1,'switch_credit_reservation':2*vcs,'switch_egress_packet_queue':2*vcs,'upli_receive_fifo':2*vcs*ports}.items())
