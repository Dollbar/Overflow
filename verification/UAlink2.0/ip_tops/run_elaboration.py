...
Add --transactions to elaborate the causal Read core; add --synth for generic gate synthesis (no technology mapping or STA).
...
def main():
    ...
    p.add_argument('--label',required=True);p.add_argument('--synth',action='store_true');p.add_argument('--transactions',action='store_true')
    ...
    result=dict(complete=False,endpoint_transaction_mode=int(a.transactions),originator_capacity=a.originator_capacity,completer_capacity=a.completer_capacity,mode='generic_synthesis' if a.synth else 'elaboration',technology_sta=False,functional_completeness=False,sources={str(f):hashlib.sha256(f.read_bytes()).hexdigest() for f in sources},results=[])
    for top in ('ualink_endpoint_top','ualink_switch_top'):
        ...
        if a.transactions and top=='ualink_endpoint_top':command+=f'chparam -set TRANSACTION_MODE 1 -set ORIGINATOR_CAPACITY {a.originator_capacity} -set COMPLETER_CAPACITY {a.completer_capacity} ualink_endpoint_top\n'
