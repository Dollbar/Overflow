#!/usr/bin/env python3
"""Run: python3 verification/upli_channels/run_endpoint_memory_top.py --label NEW --kd28-root PATH [--ports 1|2|4] [--static].
Artifacts: ROOT/build/verification/upli_channels/endpoint_memory_top/NEW snapshots/logs/hashes.
Next: connect real request/response contexts and backend execution; full RAS remains open.
"""
import argparse,hashlib,json,re,subprocess,time,sys
...
from endpoint_memory_top_reference import vectors
TOP='upli_endpoint_ip_top'
def main():
 p=argparse.ArgumentParser(description=__doc__);p.add_argument('--label',required=True);p.add_argument('--kd28-root',type=Path,required=True);p.add_argument('--ports',type=int,choices=(1,2,4));p.add_argument('--tl-mode',type=int,choices=(0,1),default=0);p.add_argument('--static',action='store_true');p.add_argument('--context-only',action='store_true');p.add_argument('--legacy',action='store_true');p.add_argument('--fault',choices=('issue','command_tag','result_data','final','release'))
 a=p.parse_args();a.red=False;enable=0 if a.legacy else 1;memory_enable=0 if a.legacy or a.context_only else 1
 if not re.fullmatch('[A-Za-z0-9_-]+',a.label):p.error('safe fresh label required')
 stage=ROOT/'build/verification/upli_channels/endpoint_memory_top'/a.label;stage.mkdir(parents=True,exist_ok=False);src=stage/'source';src.mkdir()
 ...
 hashes={};files=[]
 for f in paths+[HERE/('endpoint_memory_top_legacy_tb.sv' if a.legacy else 'endpoint_memory_top_context_tb.sv' if a.context_only else 'endpoint_memory_top_tb.sv'),Path(__file__),HERE/'endpoint_memory_top_reference.py',ROOT/'third_party/kd28_dependency.json']:
  ...
  if a.fault and f.stem==TOP:
   edits={
    'issue':[(".i_issue_ready(context_issue_ready)",".i_issue_ready(i_backend_issue_ready)")],
    'command_tag':[(".i_issue_payload(o_backend_issue_payload)",".i_issue_payload(o_backend_issue_payload ^ (184'd1 << 97))")],
    'result_data':[(".i_result_data(i_memory_result_data)",".i_result_data({512'd0,i_memory_result_data[1535:0]})")],
    'final':[(".i_final_valid(i_memory_final_valid)",".i_final_valid(1'b0)")],
    'release':[(".i_release_valid(context_release_valid)",".i_release_valid(i_memory_completion_ready && o_memory_completion_valid)"),(".i_release_token(context_release_token)",".i_release_token(o_memory_completion_token)")]
   }[a.fault]
   t=b.decode()
 ...
 for ports in ([a.ports] if a.ports else [1,2,4]):
  folder=stage/f'p{ports}';folder.mkdir();commands={};ref=vectors(folder/'vectors.txt',ports)
  commands['compile']=run(folder,'compile',['iverilog','-g2012','-s','tb',f'-Ptb.PORTS={ports}',f'-Ptb.TL={a.tl_mode}','-o','sim.vvp',*files,src/('endpoint_memory_top_legacy_tb.sv' if a.legacy else 'endpoint_memory_top_context_tb.sv' if a.context_only else 'endpoint_memory_top_tb.sv')])
  ...
  passed=commands['compile']['exit']==0 and commands.get('run',{}).get('exit')==(1 if a.fault or a.red else 0) and ('IP_TOP_MISMATCH' if a.fault or a.red else ('IP_TOP_PASS' if a.legacy else 'CONTEXT_TOP_PASS' if a.context_only else 'MEMORY_TOP_PASS')) in log
  structure={}
  if a.static and not a.fault and not a.red and passed:
   for name,cmd in {
    'g2001':['iverilog','-g2001','-s',TOP,f'-P{TOP}.C_NUM_PORTS={ports}',f'-P{TOP}.C_IS_TL={a.tl_mode}',f'-P{TOP}.C_REQUEST_CONTEXT_ENABLE={enable}',f'-P{TOP}.C_MEMORY_ADAPTER_ENABLE={memory_enable}','-o','rtl.vvp',*static],
    'lint':['verilator','--lint-only','--language','1364-2001','-Wall','--top-module',TOP,f'-GC_NUM_PORTS={ports}',f'-GC_IS_TL={a.tl_mode}',f'-GC_REQUEST_CONTEXT_ENABLE={enable}',f'-GC_MEMORY_ADAPTER_ENABLE={memory_enable}',*lint],
    'yosys':['yosys','-Q','-T','-p','read_verilog '+' '.join(map(str,static))+f'; chparam -set C_NUM_PORTS {ports} -set C_IS_TL {a.tl_mode} -set C_REQUEST_CONTEXT_ENABLE {enable} -set C_MEMORY_ADAPTER_ENABLE {memory_enable} {TOP}; hierarchy -check -top {TOP}; proc; opt; check -assert; stat; write_json hierarchy.json']}.items():commands[name]=run(folder,name,cmd)
   ...
   if (folder/'hierarchy.json').exists():
    ...
    passed=passed and hier.get('upli_endpoint_request_context',0)==enable and hier.get('endpoint_memory_adapter',0)==memory_enable and structure['latches']==0 and all(hier.get(k)==v for k,v in {'upli_native_rx_channel':4,'upli_receive_tdm_monitor':1,'upli_native_rx_burst_monitor':1,'upli_endpoint_request_bridge':1,'upli_ordered_receive_channel':4,'upli_parity':36,'upli_endpoint_response_collector':1,'upli_station_tx':1,'upli_credit_bank':4,'upli_connection_side':2,'upli_rx_role_fault_controller':1}.items())
  results.append({'ports':ports,'tl_mode':a.tl_mode,'reference':ref,'passed':passed,'commands':commands,'structure':structure});print(ports,passed,log[-500:],flush=True)
 guards=[]
 if a.static and not a.red and not a.fault:
  folder=stage/'parameter_guards';folder.mkdir()
  for context_mode,memory_mode in ((0,1),(1,2)):
   name=f'context{context_mode}_memory{memory_mode}'
   command=run(folder,name,['iverilog','-g2001','-s',TOP,f'-P{TOP}.C_REQUEST_CONTEXT_ENABLE={context_mode}',f'-P{TOP}.C_MEMORY_ADAPTER_ENABLE={memory_mode}','-o',name+'.vvp',*static])
   log=(folder/(name+'.log')).read_text();guards.append({'mode':[context_mode,memory_mode],'command':command,'passed':command['exit']!=0 and 'upli_endpoint_memory_mode_invalid' in log})
 result={'parameter_guards':guards,'python_optimization':sys.flags.optimize,'context_only':a.context_only,'passed':all(c['passed'] for c in results) and all(g['passed'] for g in guards),'fault':a.fault,'red':a.red,'legacy':a.legacy,'cases':results,'sources_sha256':hashes,'artifacts_sha256':{str(f.relative_to(stage)):hashlib.sha256(f.read_bytes()).hexdigest() for f in stage.rglob('*') if f.is_file()}}
