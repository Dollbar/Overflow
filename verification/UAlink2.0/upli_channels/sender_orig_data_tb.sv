...
module upli_burst_sender_tb; // 完整暂存所有权及真实银行集成的自检模块。
    ...
    upli_burst_sender #( // 实际控制、寄存暂存和两个银行组成的 DUT。
        ...
        .o_data_valid(raw_data_valid), .o_data_port(raw_data_port), .o_data_vc(raw_data_vc), .o_data_pool(raw_data_pool), .o_data_offset(raw_data_offset), .o_data_last(raw_data_last), // 真实首尾元数据。
        .o_data_payload(raw_data_payload), .o_data_byte_enable(raw_data_byte_enable), .o_data_error(raw_data_error), // 完整 OrigData 字段。
    ...
    wire [0:0] raw_data_valid;
    wire [1:0] raw_data_port;
    wire [511:0] raw_data_payload;
    wire [63:0] raw_data_byte_enable;
    wire [1:0] raw_data_offset;
    wire [0:0] raw_data_last;
    wire [0:0] raw_data_error;
    wire [1:0] raw_data_vc;
    wire [0:0] raw_data_pool;
    wire valid_parity,be_parity,fields_parity;
    wire [7:0] data_parity;
    integer parity_group,parity_bit;
    reg parity_expected;
    upli_orig_data_channel Native_Inst(.i_valid(raw_data_valid),.i_port_id(raw_data_port),.i_data(raw_data_payload),.i_byte_en(raw_data_byte_enable),.i_offset(raw_data_offset),.i_last(raw_data_last),.i_error(raw_data_error),.i_vc(raw_data_vc),.i_pool(raw_data_pool),.o_orig_data_valid(data_valid),.o_orig_data_port_id(data_port),.o_orig_data(data_payload),.o_orig_data_byte_en(data_byte_enable),.o_orig_data_offset(data_offset),.o_orig_data_last(data_last),.o_orig_data_error(data_error),.o_orig_data_vc(data_vc),.o_orig_data_pool(data_pool),.o_orig_data_valid_parity(valid_parity),.o_orig_data_parity(data_parity),.o_orig_data_byte_en_parity(be_parity),.o_orig_data_fields_parity(fields_parity));
    initial begin // 完整有限轨迹及独立历史 journal 自检。
        ...
        fields = $fscanf(fd, "%h %h %h %h\n", stimulus, expected, expected_before, expected_after); // 每行显式带沿前与沿后状态。
        while (fields == 4) begin // 仅接受完整的四字段记录。
            #(C_HALF_PERIOD_PS-1); // 给组合输入到事件的路径稳定时间。
            if (valid_parity !== data_valid || fields_parity !== ^{data_last,data_error,data_offset,data_port,data_vc,data_pool} || be_parity !== ^data_byte_enable) $fatal(1,"ORIG_INTEGRATION_CONTROL_PARITY row=%0d",row);
            for(parity_group=0;parity_group<8;parity_group=parity_group+1) begin
                parity_expected=0;
                for(parity_bit=0;parity_bit<64;parity_bit=parity_bit+1) parity_expected=parity_expected ^ data_payload[parity_group*64+parity_bit];
                if(data_parity[parity_group] !== parity_expected) $fatal(1,"ORIG_INTEGRATION_DATA_PARITY row=%0d",row);
            end
            ...
            fields = $fscanf(fd, "%h %h %h %h\n", stimulus, expected, expected_before, expected_after); // 消费下一份独立记录。
        end // 结束全部轨迹。
