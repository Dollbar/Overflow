 ...
 for ports in ([a.ports] if a.ports else [1,2,4]):
  ...
  if a.static and not a.fault and not a.red and passed:
   for name,cmd in {
    ...
    'yosys':['yosys','-Q','-T','-p','read_verilog '+' '.join(map(str,static))+f'; chparam -set C_NUM_PORTS {ports} -set C_IS_TL {a.tl_mode} {TOP}; hierarchy -check -top {TOP}; proc; opt; check -assert; stat; write_json hierarchy.json']}.items():commands[name]=run(folder,name,cmd)
