...
Endpoint使用128-bit角色内位图，已分配slot0–108（109个）；Switch使用128-bit角色内位图，已分配slot0–126（127个）。每个最初planned条目自带 `feature_slots`，晋升后仍保留。Endpoint的11/12、Switch的53已由typed子集实现替代，事务核、发起器、Tag表、接收器、响应组装器和completer进一步晋升后，当前壳位分别93/118个。shared模块通过同一稳定module id映射到各角色独立slot。未分配位和已移除壳的slot为0，其余未实现壳位为1；零位仅表示该壳不再待实现，不证明相应子系统或集成完整符合性。
...
| `rtl/upli/upli_read_response_channel.v` | E/S / existing_partial / E:20/S:2 | Typed native Read Response TX fields and full data parity; Single/Multi scheduling, receive and credit remain incomplete. |
