# Run: UALINK_CLOCK_PERIOD_PS=640 UALINK_LIBERTY=/authorized/corner.lib UALINK_NETLIST=/exact/mapped.v sta -exit scripts/sta_dl_basic_control.tcl
...
if {[catch {
    ...
    link_design dl_basic_message_control
    create_clock -name DL_BASIC -period $period_ns [get_ports i_clk]
    set_clock_uncertainty -setup 0.032 [get_clocks DL_BASIC]
    set_clock_uncertainty -hold 0.010 [get_clocks DL_BASIC]
    set_clock_transition 0.050 [get_clocks DL_BASIC]
    set_input_delay -clock DL_BASIC -max 0.128 [all_inputs -no_clocks]
    set_input_delay -clock DL_BASIC -min 0.020 [all_inputs -no_clocks]
    set_input_transition 0.050 [all_inputs -no_clocks]
    set_output_delay -clock DL_BASIC -max 0.128 [all_outputs]
    set_output_delay -clock DL_BASIC -min -0.020 [all_outputs]
    set_load 0.005 [all_outputs]
    puts "DL_BASIC_PROFILE configured_period_ps=$env(UALINK_CLOCK_PERIOD_PS) declared_state_bits=156 timer_bits=$timer_bits"
    report_units
    check_setup -verbose
    ...
    puts "PASS dl_basic_message_control setup/hold at period_ns=$period_ns; prelayout budget only"
} reason]} {
    puts stderr "FAIL DL BASIC STA: $reason"
    exit 1
}
exit 0
