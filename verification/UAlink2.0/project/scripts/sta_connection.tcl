# Run with: UALINK_LIBERTY=... UALINK_NETLIST=... UALINK_PERIOD_NS=0.640 sta -exit scripts/sta_connection.tcl
# Output: stdout contains constraints diagnostics, setup/hold paths and worst slack.
# Next: repeat for other declared corners/modes; these are pre-layout estimates.
# IO/load/uncertainty values below are local budgeting assumptions, not PHY specs.
if {[catch {
    ...
    link_design upli_connection_side
    ...
    puts "PASS connection setup/hold at period_ns=$env(UALINK_PERIOD_NS); prelayout budget only"
} reason]} {
    puts stderr "FAIL connection STA: $reason"
    exit 1
}
exit 0
