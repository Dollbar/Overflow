# Run: sta -exit scripts/sta_burst_sender.tcl with UALINK_LIBERTY,
...
# Next: run check_payload_sta_report.py and bind to the exact mapped artifact.
...
if {[catch {
    ...
    puts "PASS burst_sender setup/hold at period_ns=$env(UALINK_PERIOD_NS); prelayout budget only"
} reason]} {
    puts stderr "FAIL burst sender STA: $reason"
    exit 1
}
exit 0
