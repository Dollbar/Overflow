# Run: sta -exit scripts/size_uart_tx_source.tcl with explicit UALINK_LIBERTY,
...
if {[catch {
    ...
    link_design dl_uart_tx_source
    create_clock -name UART_TX -period 0.640 [get_ports i_clk]
    ...
    for {set round 0} {$round < 80} {incr round} {
        ...
        foreach name [lsort -unique $names] {
            ...
            # Only the same positive-edge DFQ flop family may change drive strength;
            # all other sequential and clock-gating cells are excluded. Only the
            ...
            if {([regexp {^(DF|SD|SDF|LH|LN|CKLN|CKLH)} $family] && $family ne "DFQ") ||
    ...
    close $journal_file
    puts "DONE UART source sizing changes=$changes; equivalence and all-corner STA still required"
} reason]} {
    puts stderr "FAIL UART source sizing: $reason"
    exit 1
}
exit 0
