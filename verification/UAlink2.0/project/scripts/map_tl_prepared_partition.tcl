# Run: UALINK_LIBERTY=... UALINK_BUILD_DIR=... UALINK_WIDTH=8|16 yosys -c scripts/map_tl_prepared_partition.tcl
...
file mkdir $result
foreach name {tl_prepared_partition tl_control_decode tl_control_tenure} {
    yosys read_verilog [file join $root rtl tl $name.v]
}
yosys chparam -set WIDTH $env(UALINK_WIDTH) tl_prepared_partition
yosys synth -flatten -noabc -top tl_prepared_partition
yosys rename -top tl_prepared_partition
...
yosys hierarchy -check -top tl_prepared_partition
yosys check -assert
