# Run: UALINK_DEPTH=3 UALINK_CONTENT=1 UALINK_KD28_ROOT=/authorized/models UALINK_BUILD_DIR=/new/proof yosys -Q -T -c scripts/prove_uart_rx.tcl
# Outputs: actual-memory instrumentation, reset/induction logs and counterexample VCDs on failure.
# Next: bind source/model hashes and exercise real faults; content=0 excludes stored-word ordering.
...
if {$content ni {0 1}} {error "Content mode must be 0 or 1"}
...
yosys read_verilog [file join $root verification formal upli_receive_word_invariant.v]
yosys read_verilog [file join $root verification formal uart_rx_properties.v]
yosys chparam -set C_DEPTH $depth -set C_CONTENT $content uart_rx_properties
yosys prep -top uart_rx_properties -flatten
yosys opt -full
yosys check -assert
yosys select -assert-none t:KD28_* {t:$mem*} {t:$any*} {t:$assume} a:blackbox=1
yosys select -assert-count 7 uart_rx_properties/i:*
yosys select -clear
yosys write_json [file join $result properties.json]
yosys sat -verify -seq 3 -timeout 120 -set-at 1 i_rstn 0 -prove o_violation 0 -prove-skip 1 -show-inputs -show o_groups -dump_vcd [file join $result reset_counterexample.vcd] uart_rx_properties
puts "UART_RX_RESET_BASE_PROVED"
yosys sat -verify -seq 2 -timeout 120 -set-at 1 o_violation 0 -prove o_violation 0 -prove-skip 1 -show-inputs -show o_groups -dump_vcd [file join $result induction_counterexample.vcd] uart_rx_properties
puts "UART_RX_FULL_INDUCTION_PROVED"
puts "UART_RX_FUNCTIONAL_PROVED depth=$depth content=$content outputs=22 output_bits=124 control_state_bits=20"
