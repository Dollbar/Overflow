# Run through make -f scripts/burst.mk sta-burst-control with explicit LIB_ROOT.
# Output: diagnostics, critical paths, setup/hold and library-limit reports.
# Next: review five declared cell corners and both original clock modes.
# Local digital boundary budgets, ideal clock and no routed parasitics/PHY claims.
if {[catch {
    ...
    puts "PASS burst_control setup/hold at period_ns=$env(UALINK_PERIOD_NS); prelayout budget only"
} reason]} {
    puts stderr "FAIL burst control STA: $reason"
    exit 1
}
exit 0
