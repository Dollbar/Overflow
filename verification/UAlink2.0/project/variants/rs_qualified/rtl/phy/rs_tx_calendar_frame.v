...
wire flag_frame_active_raw; // 信号接收真实帧控制器未屏蔽的活跃寄存状态
wire flag_frame_done_raw; // 信号接收真实帧控制器完整组合完成资格
...
assign o_next_phase = o_frame_done ? wire_phase_after : o_phase; // 真实完整提交仅选择提前计算的下一相位，复位或取消时保持当前相位
...
assign wire_next_kind = flag_frame_done_raw ? wire_kind_after : wire_kind_current; // 真实整帧完成仅做末级候选选择，不再进入相位译码链
...
assign o_flit_ready = o_next_valid && ((!flag_frame_active_raw && (wire_kind_current == 2'd0)) || (flag_frame_done_raw && (wire_kind_after == 2'd0))); // 空闲按当前DL槽预约，真实整帧完成按下一DL槽预约
assign o_flit_reserve = o_flit_ready && i_flit_valid; // DL预约资格已保证合法Data命令，取得实际描述符才提交预约
...
rs_tx_frame_control #(.C_BLOCKS(C_BLOCKS)) u_frame ( // 连接已验证完整帧控制器及其真实格式器
    ...
    .o_active_raw(flag_frame_active_raw), // 连接真实活跃资格，公开握手仍受本地恢复屏蔽
    .o_frame_done_raw(flag_frame_done_raw), // 连接真实格式器和组末握手的组合资格
