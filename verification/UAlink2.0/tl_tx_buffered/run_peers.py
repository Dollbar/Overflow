"""Run: python3 verification/tl_tx_buffered/run_peers.py --kd28-root PATH
...
R=(lambda _ualink_file: next(((_ualink_dir / (_ualink_dir / '.ualink-root').read_text(encoding='utf-8').strip()).resolve() for _ualink_dir in _ualink_file.parents if (_ualink_dir / '.ualink-root').is_file()), Path(__file__).resolve().parents[2]))(__import__('pathlib').Path(__file__).resolve());sys.path.insert(0,str(R/'model/tl'))
from credit_context import decode_context
p=argparse.ArgumentParser(description=__doc__);p.add_argument('--kd28-root',type=Path,required=True);p.add_argument('--blocked',choices=('none','request','response'),default='none');p.add_argument('--label');p.add_argument('--single',action='store_true');p.add_argument('--replace',type=Path);p.add_argument('--bank-depth',type=int,default=3);p.add_argument('--header-depth',type=int,default=2);p.add_argument('--tag-pattern',action='store_true');a=p.parse_args();S=R/'build/verification/tl_tx_buffered'/(a.label or a.blocked);S.mkdir(parents=True,exist_ok=False)
...
for width,auth,shared,delay in configs:
    B=S/f'w{width}_a{auth}_s{shared}_l{delay}';B.mkdir();streams=[[fixtures(auth,e,c) for c in (0,1)] for e in (0,1)];caps=[2]*10+[1 if blocked==0 else 4]*5+[1 if blocked==1 else 4]*5;depth=2*sum(caps);bits=20*(width+1);cw=depth.bit_length();hcw=a.header_depth.bit_length();dcw=a.bank_depth.bit_length()+1;decl=[];assigns=[];reads=[];data_cases=[];header_checks=[];limits=[];end_condition=[]
    for e in (0,1):
        for c in (0,1):
            ...
            assigns.append(f'assign hd[{e}][{c*256}+:256]=wh[{e}][{c}]<{nh}?headers{suffix}[wh[{e}][{c}]]:0;assign hv[{e}][{c}]=done[{e}]&&peer_done[{e}]&&(wh[{e}][{c}]<{nh})&&(cycle>=next_h[{e}][{c}]);\nassign dv[{e}][{c*2}+:2]=(cycle<next_d[{e}][{c}])?2\'d0:((wd[{e}][{c}]+1<{nd})&&(wd[{e}][{c}]%7!=1))?2\'d2:(wd[{e}][{c}]<{nd})?2\'d1:2\'d0;\nassign d0[{e}][{c*256}+:256]=wd[{e}][{c}]<{nd}?data{suffix}[wd[{e}][{c}]]:0;assign d1[{e}][{c*256}+:256]=wd[{e}][{c}]+1<{nd}?data{suffix}[wd[{e}][{c}]+1]:0;')
            tag=f"32'h{(e<<24)|(c<<16):x}|wh[{e}][{c}]" if a.tag_pattern else "256'd0"
            assigns.append(f'assign tags_in[{e}][{c*256}+:256]={tag};assign tags_valid[{e}][{c}]='+ (f'(cycle>=next_h[{e}][{c}]+2)' if a.tag_pattern else "1'b1")+';')
            if a.tag_pattern and auth:header_checks.append(f'if(ht[{e}][{c}]&&tx[{e}][511:256]!== (256\'h{(e<<24)|(c<<16):x}|hi[{e}][{c}]))$fatal(1,"stored AuthTags not paired with header");')
...
{chr(10).join(decl)}
integer hi[0:1][0:1],di[0:1][0:1],wh[0:1][0:1],wd[0:1][0:1],next_h[0:1][0:1],next_d[0:1][0:1];integer hwait=0,dwait=0;
wire [3:0] da[0:1];wire [1:0] hr[0:1],dr[0:1],input_error[0:1];wire [{2*hcw-1}:0] hcount[0:1];wire [{2*dcw-1}:0] dcount[0:1];
...
integer trace,qtrace;initial begin trace=$fopen("{B}/trace.txt","w");qtrace=$fopen("{B}/queue_trace.txt","w");end
always @(posedge clk)begin
 if(!rstn)begin
 for(e=0;e<2;e=e+1)begin for(c=0;c<2;c=c+1)begin hi[e][c]=0;di[e][c]=0;wh[e][c]<=0;wd[e][c]<=0;next_h[e][c]<=0;next_d[e][c]<=0;end stalled[e]=0;held_flit[e]=0;held_msg[e]=0;for(j=0;j<{delay};j=j+1)begin lv[e][j]<=0;link[e][j]<=0;message[e][j]<=0;end end
 end else begin
 ...
 $fdisplay(qtrace,"%0d %0d %0d %0d %0d %0d %0d %0d %0d %0d %0d %0d %0d %0d %0d %0d",cycle,e,c,wh[e][c],wd[e][c],hi[e][c],di[e][c],hv[e][c],hr[e][c],dv[e][c*2+:2],dr[e][c],ht[e][c],dt[e][c*2+:2],hcount[e][c*{hcw}+:{hcw}],dcount[e][c*{dcw}+:{dcw}],da[e][c*2+:2]);
 if(hcount[e][c*{hcw}+:{hcw}]!== (wh[e][c]-hi[e][c])||dcount[e][c*{dcw}+:{dcw}]!== (wd[e][c]-di[e][c]))$fatal(1,"actual Tx capacity conservation");
 if(hv[e][c]&&hr[e][c])begin wh[e][c]<=wh[e][c]+1;next_h[e][c]<=cycle+(wh[e][c]%4);end
...
$display("PASS actual channels width={width} auth={auth} shared={shared} delay={delay} blocked={a.blocked} cycles=%0d stored=%0d fc=%0d cross_tail=%0d",cycle,stored,fc_total,cross_tail);
