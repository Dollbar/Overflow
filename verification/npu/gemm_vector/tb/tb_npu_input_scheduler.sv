...
`endif
    ...
    function automatic npu_task_descriptor_t make_descriptor(
        ...
        input logic [3:0] anchor_x,
        input logic [3:0] anchor_y
    );
        ...
        begin
            ...
            descriptor.tile_anchor_x = anchor_x;
            descriptor.tile_anchor_y = anchor_y;
    ...
    initial begin
        ...
        // A square descriptor derives one common Tile span for A, B, and C.
        scenario_id = 8'd1;
        descriptor = make_descriptor(16'h0101, 16'd48, 4'd0, 4'd0);
        ...
        if ((gemm_log[0].job_id != 16'h0101) ||
            (gemm_log[0].tile_span != 5'd3) ||
            (gemm_log[0].matrix_size != 16'd48) ||
            (gemm_log[0].activation_format != FP8_E4M3) ||
            (gemm_log[0].weight_format != FP8_E5M2) ||
        ...
        scenario_id = 8'd2;
        descriptor = make_descriptor(16'h0201, 16'd16, 4'd0, 4'd0);
        descriptor.wait_event_valid = 1'b1;
        descriptor.wait_event_id = 8'd9;
        send_task(descriptor);
        descriptor = make_descriptor(16'h0202, 16'd16, 4'd4, 4'd0);
        send_task(descriptor);
        ...
        scenario_id = 8'd5;
        base_weight = weight_count;
        descriptor = make_descriptor(16'h0501, 16'd16, 4'd12, 4'd0);
        send_task(descriptor);
        ...
        // Descriptor contents are compiler-owned and pass through without a
        // scheduler-side legality gate.
        scenario_id = 8'd6;
        base_gemm = gemm_count;
        base_activation = activation_count;
        base_weight = weight_count;
        base_result = result_count;
        base_status = status_count;
        descriptor = make_descriptor(16'h0601, 16'd20, 4'd0, 4'd12);
        descriptor.version = 4'd0;
        descriptor.operation = npu_task_operation_e'(3'd7);
        descriptor.activation_base_offset = 32'h0000_1001;
        descriptor.weight_base_offset = 32'h0000_4002;
        descriptor.output_base_offset = 32'h0000_8003;
        send_task(descriptor);
        wait_for_gemm_count(base_gemm + 1);
        @(negedge clk_i);
        if ((gemm_log[base_gemm].job_id != 16'h0601) ||
            (gemm_log[base_gemm].matrix_size != 16'd20) ||
            (gemm_log[base_gemm].tile_span != 5'd1) ||
            (activation_log[base_activation].base_offset != 32'h0000_1001) ||
            (weight_log[base_weight].base_offset != 32'h0000_4002) ||
            (result_log[base_result].base_offset != 32'h0000_8003) ||
            protocol_error_o || (status_count != base_status)) begin
            $fatal(1, "FAIL: scheduler inspected or rejected compiler-owned fields");
        end
        complete_task(gemm_log[base_gemm].tag, 1'b1);
        ...
        scenario_id = 8'd7;
        descriptor = make_descriptor(16'h0701, 16'd16, 4'd0, 4'd12);
        descriptor.wait_event_valid = 1'b1;
        ...
        descriptor = make_descriptor(16'h0702, 16'd16, 4'd0, 4'd12);
        send_task(descriptor);
        wait_for_gemm_count(base_gemm + 1);
        complete_task(gemm_log[base_gemm].tag, 1'b1);
        wait_for_status_count(base_status + 2);
        ...
        scenario_id = 8'd8;
        descriptor = make_descriptor(16'h0801, 16'd16, 4'd4, 4'd12);
        send_task(descriptor);
        wait_for_gemm_count(base_gemm + 2);
        ...
        if (status_log[base_status + 2].code !=
        ...
        @(negedge clk_i);
        clear_i = 1'b0;
        descriptor = make_descriptor(16'h0802, 16'd16, 4'd8, 4'd12);
        send_task(descriptor);
        wait_for_gemm_count(base_gemm + 3);
        ...
        $display("PASS: npu_input_scheduler input-buffer compiler-owned placement, dependency bypass, atomic six-way command split, backpressure, completion, clear/reset checks=%0d",
                 check_count);
        $finish;
    end
