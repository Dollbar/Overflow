#!/usr/bin/env python3
"""Generate deterministic vectors for the Softmax-domain FP32 reciprocal."""

from __future__ import annotations

import argparse
import math
import random
import struct
from pathlib import Path


FP32_CANONICAL_NAN = 0x7FC00000
FP32_POSITIVE_INFINITY = 0x7F800000
FP32_ONE = 0x3F800000
FP32_SIXTEEN = 0x41800000
ANCHORS_Q23 = (
    0x800000,
    0x787878,
    0x71C71C,
    0x6BCA1B,
    0x666666,
    0x618618,
    0x5D1746,
    0x590B21,
    0x555555,
    0x51EB85,
    0x4EC4EC,
    0x4BDA13,
    0x492492,
    0x469EE6,
    0x444444,
    0x421084,
    0x400000,
)


def float_to_bits(value: float) -> int:
    return struct.unpack(">I", struct.pack(">f", value))[0]


def bits_to_float(bits: int) -> float:
    return struct.unpack(">f", struct.pack(">I", bits & 0xFFFFFFFF))[0]


def reciprocal_approx_bits(data_bits: int, invalid_in: int) -> tuple[int, int]:
    sign = (data_bits >> 31) & 1
    exponent = (data_bits >> 23) & 0xFF
    fraction = data_bits & 0x7FFFFF
    magnitude = data_bits & 0x7FFFFFFF
    is_nan = exponent == 0xFF and fraction != 0
    is_inf = exponent == 0xFF and fraction == 0
    is_zero = magnitude == 0

    invalid = int(bool(invalid_in))
    if is_nan:
        return FP32_CANONICAL_NAN, 1
    if sign:
        return FP32_CANONICAL_NAN, 1
    if is_inf:
        return 0, invalid
    if is_zero:
        return FP32_POSITIVE_INFINITY, 1
    if data_bits < FP32_ONE:
        return FP32_ONE, 1
    if data_bits > FP32_SIXTEEN:
        return 0, 1

    segment = fraction >> 19
    remainder = (fraction >> 7) & 0xFFF
    high = ANCHORS_Q23[segment]
    low = ANCHORS_Q23[segment + 1]
    product = (high - low) * remainder
    increment = int(
        ((product >> 11) & 1) != 0
        and ((product & 0x7FF) != 0 or ((product >> 12) & 1) != 0)
    )
    correction = (product >> 12) + increment
    interpolated = high - correction
    unbiased_exponent = exponent - 127

    if interpolated == 0x800000:
        significand = interpolated
        biased_exponent = 127 - unbiased_exponent
    else:
        significand = (interpolated << 1) & 0xFFFFFF
        biased_exponent = 126 - unbiased_exponent
    result = ((biased_exponent & 0xFF) << 23) | (significand & 0x7FFFFF)
    return result, invalid


def directed_inputs() -> list[tuple[int, int]]:
    values = [
        0.0,
        -0.0,
        2.0**-126,
        0.5,
        0.99999994,
        1.0,
        1.0001,
        1.03125,
        1.0625,
        1.5,
        2.0,
        3.0,
        4.0,
        8.0,
        15.999,
        16.0,
        16.001,
        32.0,
        -1.0,
        -16.0,
    ]
    vectors = [(float_to_bits(value), 0) for value in values]
    vectors.extend(
        [
            (0x7F800000, 0),
            (0xFF800000, 0),
            (0x7FC00001, 0),
            (0xFFC12345, 0),
            (float_to_bits(1.0), 1),
            (float_to_bits(16.0), 1),
            (0x00000001, 0),
            (0x80000001, 0),
            (0x3F800001, 0),
            (0x3F80007F, 0),
            (0x3F800080, 0),
        ]
    )
    return vectors


def generate_inputs(count: int, seed: int) -> list[tuple[int, int]]:
    rng = random.Random(seed)
    vectors = directed_inputs()
    while len(vectors) < count:
        selection = rng.randrange(10)
        if selection < 7:
            value = 2.0 ** rng.uniform(0.0, 4.0)
            bits = float_to_bits(value)
        elif selection == 7:
            bits = float_to_bits(rng.random())
        elif selection == 8:
            bits = float_to_bits(16.0 + 64.0 * rng.random())
        else:
            bits = rng.getrandbits(32)
        vectors.append((bits, int(rng.randrange(32) == 0)))
    return vectors[:count]


def write_vectors(output: Path, count: int, seed: int) -> None:
    vectors = generate_inputs(count, seed)
    output.parent.mkdir(parents=True, exist_ok=True)
    maximum_relative_error = 0.0
    maximum_absolute_error = 0.0
    measured_count = 0
    with output.open("w", encoding="ascii") as handle:
        for data_bits, invalid_in in vectors:
            expected_bits, invalid_out = reciprocal_approx_bits(data_bits, invalid_in)
            packed = (
                (data_bits << 34)
                | ((invalid_in & 1) << 33)
                | (expected_bits << 1)
                | (invalid_out & 1)
            )
            handle.write(f"{packed:017x}\n")

            value = bits_to_float(data_bits)
            if math.isfinite(value) and 1.0 <= value <= 16.0:
                approximate = bits_to_float(expected_bits)
                exact = 1.0 / value
                absolute_error = abs(approximate - exact)
                relative_error = absolute_error / exact
                maximum_absolute_error = max(maximum_absolute_error, absolute_error)
                maximum_relative_error = max(maximum_relative_error, relative_error)
                measured_count += 1

    if maximum_relative_error > 0.005:
        raise RuntimeError(
            f"relative error {maximum_relative_error:.8f} exceeds 0.5% contract"
        )
    print(
        "Generated "
        f"{len(vectors)} fp32_recip_pipeline vectors; measured={measured_count} "
        f"max_relative_error={maximum_relative_error:.8f} "
        f"max_absolute_error={maximum_absolute_error:.8e}"
    )


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--output", type=Path, required=True)
    parser.add_argument("--count", type=int, default=4096)
    parser.add_argument("--seed", type=int, default=0x5EC1)
    args = parser.parse_args()
    write_vectors(args.output, args.count, args.seed)


if __name__ == "__main__":
    main()
