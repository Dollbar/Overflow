...
module tb_kdlink_hierarchical_collective;
    ...
    initial begin
        ...
        @(negedge clk);
        @(negedge clk);
        query_group_id = 32'h2550_0001;
        query_epoch = 8'd25;
        descriptor_transaction_id = 64'h2550_0000_0000_0001;
        descriptor_source_domain = 8'd255;
        descriptor_destination_domain = 8'd255;
        local_domain = 8'd255;
        #0.1;
        if (!group_found || !group_local_member || group_count != 9'd1 || group_root != 8'd255) $fatal(1, "single-member high-domain group configuration failed");
        run_descriptor(`KDL_OPCODE_ALL_REDUCE, 8'd255, 0);
        config_index = 3'd0;
        config_group_id = 32'h1122_3344;
        config_epoch = 8'd19;
        config_mask = 256'd0;
