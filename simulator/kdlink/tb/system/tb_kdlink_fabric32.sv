...
module tb_kdlink_fabric32; // 定义八 plane 五百一十二 slice permutation 自校验测试
    ...
    kdlink_fabric32 u_dut ( // 实例化三十二节点八 plane fabric
    ...
    initial begin // 执行八 plane 全 slice 连续 permutation
        ...
        $display("TB_KDLINK_FABRIC32_PASS cycles=%0d nodes=32 planes=8 slices_per_node=16 flits_per_cycle=512 per_npu_GBps=1024.000 system_GBps=32768.000 bubbles=0 source_stalls=0", MEASURE_CYCLES); // 报告全系统逻辑 payload 实测吞吐
        $finish; // 结束测试
    end // 结束主测试流程
    initial begin // 设置仿真超时
        #5000; // 等待最大测试时长
        $fatal(1, "KDLink fabric32 timeout"); // 超时失败
    end // 结束超时保护
