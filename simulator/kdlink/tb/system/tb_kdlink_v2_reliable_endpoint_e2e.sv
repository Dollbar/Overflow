...
module tb_kdlink_v2_reliable_endpoint_e2e;
    localparam integer PACKETS = 80;
    localparam integer FAULT_SEQUENCE = 0;
    localparam logic [11:0] PACKETS_12 = 12'd80;
    logic phy_clk;
    logic core_a_clk;
    logic core_b_clk;
    logic rst_n;
    ...
    kdlink_v2_reliable_endpoint #(
        ...
        .phy_clk_i(phy_clk), .phy_rst_n_i(rst_n),
        .local_node_i(5'd1), .link_epoch_i(8'h2a),
    ...
    kdlink_v2_reverse_channel_model #(.PROPAGATION_CYCLES(4)) u_reverse_link (
    ...
    always @(posedge core_a_clk) begin
        if (rst_n && a_commit_valid && a_commit_last) begin
            ...
            if (a_commit_header[81:70] >= PACKETS_12)
                $fatal(1, "A commit sequence out of range");
            if (a_seen[a_commit_header[76:70]])
                $fatal(1, "A duplicate commit seq=%0d", a_commit_header[81:70]);
            a_seen[a_commit_header[76:70]] <= 1'b1;
    ...
    always @(posedge core_b_clk) begin
        if (rst_n && b_commit_valid && b_commit_last) begin
            ...
            if (b_commit_header[81:70] >= PACKETS_12)
                $fatal(1, "B commit sequence out of range");
            if (b_seen[b_commit_header[76:70]])
                $fatal(1, "B duplicate commit seq=%0d", b_commit_header[81:70]);
            b_seen[b_commit_header[76:70]] <= 1'b1;
    ...
    initial begin
        ...
        fork
            begin
                for (source_index_a = 0; source_index_a < PACKETS;
                    ...
                    a_tx_header[37:33] = 5'd1;
                    a_tx_header[45:38] = 8'h2a;
                    a_tx_header[57:46] = 12'h510;
                    a_tx_header[69:58] = source_index_a[11:0];
            ...
            begin
                for (source_index_b = 0; source_index_b < PACKETS;
                    ...
                    b_tx_header[37:33] = 5'd1;
                    b_tx_header[45:38] = 8'h2a;
                    b_tx_header[57:46] = 12'h520;
                    b_tx_header[69:58] = source_index_b[11:0];
        ...
        @(negedge core_a_clk);
        a_tx_header = 96'd0;
        a_tx_header[7:4] = 4'd4;
        a_tx_header[10:8] = 3'd2;
        a_tx_header[12:11] = 2'd3;
        a_tx_header[15:13] = 3'd7;
        a_tx_header[16] = 1'b1;
        a_tx_header[17] = 1'b1;
        a_tx_header[18] = 1'b1;
        a_tx_header[24:20] = 5'd0;
        a_tx_header[29:25] = 5'd1;
        a_tx_header[32:30] = 3'd7;
        a_tx_header[37:33] = 5'd1;
        a_tx_header[45:38] = 8'h2a;
        a_tx_header[57:46] = 12'h510;
        a_tx_header[69:58] = 12'd7;
        a_tx_header[81:70] = 12'd7;
        a_tx_header[87:82] = 6'd0;
        a_tx_payload = 512'd0;
        a_tx_payload[31:0] = 32'd7;
        a_tx_payload[39:32] = 8'ha0;
        a_tx_valid = 1'b1;
        @(posedge core_a_clk);
        while (!a_tx_ready) @(posedge core_a_clk);
        ...
        while (((a_duplicate_seen == 0 && b_duplicate_seen == 0) ||
                ...
                b_replay_occupancy != 0) && timeout_count < 10000) begin
            @(posedge phy_clk);
            timeout_count = timeout_count + 1;
        end
        ...
        if (a_commit_count != PACKETS || b_commit_count != PACKETS ||
            ...
            $fatal(1, "E2E final failure commits=%0d/%0d replay=%0d/%0d zero=%0d duplicate=%0d occupancy=%0d/%0d errors=%b%b%b%b%b%b",
        ...
        $display("TB_KDLINK_V2_RELIABLE_ENDPOINT_E2E_PASS packets_per_direction=%0d vcs=8 autonomous_nack=1 replay=1 exact_once=1 cumulative_credit=1 cdc=1 pcs=1 digital_serdes=1",
            PACKETS);
        $finish;
    end
    ...
    initial begin
        #60000;
        $fatal(1, "KDLink-v2 reliable endpoint E2E timeout");
    end
endmodule
