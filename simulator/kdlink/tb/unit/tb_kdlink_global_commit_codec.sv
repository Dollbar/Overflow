...
module tb_kdlink_global_commit_codec;
    ...
    initial begin
        ...
        header[18] = 1'b1;
        header = 96'd0;
        header[3:0] = `KDL_ROUTE_SCHEMA;
        header[7:4] = `KDL_MESSAGE_TYPE_ROUTE_CONTEXT;
        header[15:13] = `KDL_VC_ROLE_CONTROL;
        header[17] = 1'b1;
        header[18] = 1'b1;
        header[29:25] = 5'd4;
        header[94:88] = 7'd64;
        #0.1;
        if (!extended_header_valid || baseline_header_valid || !baseline_header_error[0]) $fatal(1, "Route Context header extension isolation failed");
        header[19] = 1'b1;
        header[15:13] = `KDL_VC_ROLE_REPLAY;
        #0.1;
        if (!extended_header_valid) $fatal(1, "replayed Route Context header was rejected");
        header[15:13] = `KDL_VC_ROLE_CONTROL;
        #0.1;
        if (extended_header_valid || !extended_header_error[7]) $fatal(1, "replayed Route Context used a non-replay VC");
