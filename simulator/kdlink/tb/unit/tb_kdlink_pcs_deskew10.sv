...
module tb_kdlink_pcs_deskew10; // 定义十 lane 固定偏斜恢复自校验测试
    ...
    kdlink_pcs_deskew10 u_dut ( // 实例化十 lane deskew FIFO
    ...
    initial begin // 驱动十 lane 零到四周期固定偏斜
        ...
        $display("TB_KDLINK_PCS_DESKEW10_PASS groups=%0d lanes=10 max_skew_cycles=4 bubbles=0 overflow=0", GROUPS); // 报告 deskew 验收结果
        $finish; // 结束测试
    end // 结束主测试流程
    initial begin // 设置仿真超时
        #500; // 等待最大测试时长
        $fatal(1, "KDLink PCS deskew timeout"); // 超时失败
    end // 结束超时保护
