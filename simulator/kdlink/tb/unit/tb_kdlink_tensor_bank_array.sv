...
module tb_kdlink_tensor_bank_array; // 定义十六 bank 全双工连续流自校验测试
    ...
    kdlink_tensor_bank_array u_dut ( // 实例化十六 bank 聚合边界
    ...
    initial begin // 执行十六 bank 全双工带宽测试
        ...
        $display("TB_KDLINK_TENSOR_BANK_ARRAY_PASS cycles=%0d banks=16 payload_bits_per_cycle=8192 tx_GBps=1024.000 rx_GBps=1024.000 full_duplex_GBps=2048.000 tx_bubbles=0 rx_bubbles=0", MEASURE_CYCLES); // 报告 K4 实测聚合吞吐
        $finish; // 结束测试
    end // 结束主测试流程
    initial begin // 设置仿真超时
        #20000; // 等待最大测试时长
        $fatal(1, "KDLink tensor bank array timeout"); // 超时失败
    end // 结束超时保护
