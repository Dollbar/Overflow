...
module tb_kdlink_context_scheduler16; // 定义十六 context scheduler 自校验测试
    ...
    kdlink_context_scheduler16 u_dut ( // 实例化十六 context scheduler
    ...
    initial begin // 执行 context 分配、错误和公平性测试
        ...
        for (context_index = 0; context_index < 16; context_index = context_index + 1) begin // 分配全部十六 context
            ...
            descriptor_i[223:192] = 32'd4096; // 写入非零 Tensor 长度
        ...
        $display("TB_KDLINK_CONTEXT_SCHEDULER16_PASS contexts=16 fairness_rounds=2 blocked_fairness=PASS duplicate=PASS malformed=PASS"); // 报告 scheduler 测试通过
        $finish; // 结束测试
    end // 结束主测试流程
    initial begin // 设置仿真超时
        #1000; // 等待最大测试时长
        $fatal(1, "KDLink context scheduler timeout"); // 超时失败
    end // 结束超时保护
