...
module tb_kdlink_slice; // 定义 KDLink slice 连续流与 CRC fault 自校验测试平台
    ...
    kdlink_slice u_slice ( // 实例化被测 KDLink codec slice
    ...
    kdlink_header_checker u_header_checker ( // 实例化 endpoint header 合法性检查器
    ...
    always @(negedge clk or negedge rst_n) begin // 检查 TX 连续输出
        if (!rst_n) begin // 检测复位有效
            ...
            if (tx_valid_o) begin // 检测 TX 输出 flit
                ...
                $fatal(1, "KDLink TX stream bubble count=%0d", tx_count); // 报告 TX 气泡错误
            end // 结束 TX 输出检查
        end // 结束正常 TX 检查
        ...
        if (!rst_n) begin // 检测复位有效
            ...
            if (rx_valid_o) begin // 检测 RX 输出 flit
                ...
                if (rx_count < GOOD_FLITS) begin // 检查正常 flit 区间
                    if (!rx_crc_good_o) $fatal(1, "KDLink good flit failed CRC index=%0d", rx_count); // 要求正常 flit CRC通过
                    if (!checked_header_valid || checked_header_error != 8'd0) $fatal(1, "KDLink header rejected index=%0d error=%02x", rx_count, checked_header_error); // 要求 header合法
                    if (rx_payload_o != expected_payload[rx_count]) $fatal(1, "KDLink payload mismatch index=%0d", rx_count); // 检查 payload 顺序与内容
                    if (rx_header_o != expected_header) $fatal(1, "KDLink full header mismatch index=%0d got=%h expected=%h", rx_count, rx_header_o, expected_header); // 检查完整九十六位 header
                    if (rx_payload_bytes_o != 7'(rx_count % 65)) $fatal(1, "KDLink payload length mismatch index=%0d", rx_count); // 检查动态有效字节数
                end else begin // 检查最后一条 fault flit
                    if (rx_crc_good_o) $fatal(1, "KDLink injected fault escaped CRC"); // 要求 fault 被 CRC 检出
                ...
                $fatal(1, "KDLink RX stream bubble count=%0d", rx_count); // 报告 RX 气泡错误
            end // 结束 RX 输出检查
        end // 结束正常 RX 检查
    ...
    initial begin // 执行连续流 stimulus
        ...
        if (tx_count != TOTAL_FLITS) $fatal(1, "KDLink TX count mismatch got=%0d", tx_count); // 检查 TX flit总数
        ...
        $display("TB_KDLINK_SLICE_PASS good_flits=%0d fault_flits=1 tx_bubbles=0 rx_bubbles=0 payload_GBps=64.000 header_error_matrix=PASS", GOOD_FLITS); // 报告 K2 连续流和错误矩阵通过
        $finish; // 结束仿真
    end // 结束主 stimulus
    initial begin // 设置 K2 仿真超时
        #15000; // 等待最大允许仿真时间
        $fatal(1, "KDLink slice timeout"); // 超时报错避免仿真挂起
    end // 结束仿真超时保护
endmodule // 结束 KDLink slice 测试平台
