...
module tb_kdlink_replay_buffer; // 定义多 flit、背压和 retry 边界测试
    ...
    kdlink_replay_buffer u_dut ( // 实例化单 slice replay buffer
    ...
    initial begin // 执行完整 replay 操作矩阵
        ...
        $display("TB_KDLINK_REPLAY_BUFFER_PASS packet_flits=16 legal_retries=7 replay_flits=%0d backpressure=1 ack_miss=1 nack_miss=1 retry_exhausted=1 window_entries=8 reuse_lengths=2,4,8,16,1 bit_exact=608", replay_checked); // 报告 replay 测试结果
        $finish; // 结束测试
    end // 结束主 stimulus
    initial begin // 设置 replay 测试超时
        #5000; // 等待最大允许仿真时间
        $fatal(1, "KDLink replay buffer timeout"); // 超时报错避免仿真挂起
    end // 结束超时保护
