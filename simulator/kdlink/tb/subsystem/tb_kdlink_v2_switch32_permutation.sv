...
module tb_kdlink_v2_switch32_permutation; // 定义双 slice 三十二端口无争用吞吐测试
    ...
    kdlink_v2_switch32 u_dut ( // 实例化双 slice KDSwitch
    ...
    initial begin // 执行双 slice permutation 连续流
        ...
        $display("TB_KDLINK_V2_SWITCH32_PERMUTATION_PASS cycles=%0d ports=32 slices=2 output_flits_per_cycle=64 payload_GBps=4096.000 bubbles=0 source_stalls=0", MEASURE_CYCLES); // 报告单 plane 聚合 payload 吞吐
        $finish; // 结束测试
    end // 结束主测试流程
    initial begin // 设置仿真超时
        #5000; // 等待最大测试时长
        $fatal(1, "KDLink-v2 switch32 permutation timeout"); // 超时失败
    end // 结束超时保护
