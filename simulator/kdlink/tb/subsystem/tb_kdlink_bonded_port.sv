...
module tb_kdlink_bonded_port; // 定义双 slice 满带宽、重排和 fault replay 自校验测试
    ...
    kdlink_bonded_port u_dut ( // 实例化双 slice bonded port
    ...
    initial begin // 执行双 slice、fault replay 和降级模式 stimulus
        ...
        $display("TB_KDLINK_BONDED_PORT_PASS normal_flits=%0d normal_GBps=128.000 degraded_flits=%0d degraded_GBps=64.000 bubbles=0 cross_slice_replay=1 exact_once=1", NORMAL_FLITS, DEGRADED_FLITS); // 报告 K3 全部验收结果
        $finish; // 结束仿真
    end // 结束主 stimulus
    initial begin // 设置 K3 仿真超时
        #40000; // 等待最大允许仿真时间
        $fatal(1, "KDLink bonded port timeout"); // 超时报错避免仿真挂起
    end // 结束超时保护
