...
module tb_kdlink_nic8_cdc; // 定义十六 bank 双向 CDC exact-data 自校验测试
    ...
    kdlink_nic8_cdc u_dut ( // 实例化异步 Tensor NIC wrapper
    ...
    initial begin // 执行十六 bank 双向异步 FIFO loopback
        ...
        @(negedge tensor_clk); tensor_rst_n = 1'b0; // 在活动完成后重新复位 Tensor 域
        @(negedge slice_clk); slice_rst_n = 1'b0; // 同步验证 slice 域运行时复位
        repeat (8) @(posedge tensor_clk); #0.01; // 等待全部异步 FIFO 指针回到安全状态
        if (active_o || cdc_error_o != 32'd0 || |result_valid_o || |link_tx_valid_o)
            $fatal(1, "CDC NIC runtime reset did not restore safe state active=%b cdc=%h result=%h tx=%h",
                active_o, cdc_error_o, result_valid_o, link_tx_valid_o);
        $display("TB_KDLINK_NIC8_CDC_PASS banks=16 flits_per_bank=%0d tensor_clock_GHz=1.250 slice_clock_GHz=1.000 exact_data=PASS cdc_errors=0 runtime_reset=1", TEST_FLITS); // 报告双向 CDC 测试通过
        $finish; // 结束测试
    end // 结束主测试流程
    initial begin // 设置仿真超时
        #5000; // 等待最大异步测试时长
        $fatal(1, "KDLink NIC8 CDC timeout"); // 超时失败
    end // 结束超时保护
