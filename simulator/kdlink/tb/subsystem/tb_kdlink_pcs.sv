...
module tb_kdlink_pcs; // 定义训练、marker、扰码和失锁自校验测试
    ...
    initial begin // 执行 training、alignment、data 和 fault 流程
        ...
        $display("TB_KDLINK_PCS_PASS training_groups=16 marker=1 data_flits=%0d blocks_per_flit=10 encoded_bits_per_cycle=660 bubbles=0 scramble_roundtrip=1 fault_loss_of_lock=1", DATA_FLITS); // 报告 K6 PCS 验收结果
        $finish; // 结束测试
    end // 结束主测试流程
    initial begin // 设置仿真超时
        #3000; // 等待最大测试时长
        $fatal(1, "KDLink PCS timeout"); // 超时失败
    end // 结束超时保护
