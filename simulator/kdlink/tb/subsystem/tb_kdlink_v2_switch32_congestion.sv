...
module tb_kdlink_v2_switch32_congestion; // 定义单 slice 全端口拥塞与背压测试
    ...
    kdlink_v2_switch_slice32 u_dut ( // 实例化单 slice switch 数据面
    ...
    initial begin // 执行公平性、全 egress、全 VC 和背压测试
        ...
        $display("TB_KDLINK_V2_SWITCH32_CONGESTION_PASS sources=32 fairness_flits=%0d directed_flits=%0d egresses=32 escape_pairs=1024 vcs=8 backpressure=1 bit_exact=640 loss=0", 32*FAIR_FLITS_PER_SOURCE, directed_received); // 报告全端口测试结果
        $finish; // 结束测试
    end // 结束主测试流程
    initial begin // 设置仿真超时
        #20000; // 等待最大测试时长
        $fatal(1, "KDLink-v2 switch32 congestion timeout"); // 超时失败
    end // 结束超时保护
