...
module tb_kdlink_v2_nic8; // 定义十六 bank NIC 协议和带宽自校验测试
    ...
    logic [511:0] descriptor_i; // 驱动 KDLink-v2 descriptor
    ...
    kdlink_v2_nic8 u_dut ( // 实例化八 plane 十六 bank NIC
    ...
    initial begin // 执行 descriptor、路由、协议字段和满带宽测试
        ...
        $display("TB_KDLINK_V2_NIC8_PASS cycles=%0d banks=16 measured_flits_per_direction=%0d active_window_GBps=1024.000 descriptor_to_finish_cycles=10001 full_operation_GBps=1023.898 tx_bubbles=0 rx_bubbles=0 opcodes=6", MEASURE_CYCLES, MEASURE_CYCLES*16); // 报告 NIC RTL 实测吞吐
        $finish; // 结束测试
    end // 结束主测试流程
    initial begin // 设置仿真超时
        #20000; // 等待最大测试时长
        $fatal(1, "KDLink-v2 NIC8 timeout"); // 超时失败
    end // 结束超时保护
