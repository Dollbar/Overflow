module kdlink_replay_window #(
    parameter integer SLOT_BITS = 10,
    parameter [15:0] TIMEOUT_CYCLES = 16'd4096
) (
    input wire clk_i,
    input wire rst_n_i,
    input wire clear_i,
    ...
    reg [15:0] last_send_time_q [0:SLOTS-1];
    reg [15:0] time_q;
    reg [SLOT_BITS-1:0] scan_slot_q;
    reg store_active_q;
    ...
    assign timeout_due = slot_valid_q[scan_slot_q] &&
        slot_complete_q[scan_slot_q] && !replay_active_q &&
        !replay_output_valid_q &&
        ((time_q - last_send_time_q[scan_slot_q]) >= TIMEOUT_CYCLES) &&
        !(ack_release && (ack_slot == scan_slot_q));
    ...
    always @(posedge clk_i or negedge rst_n_i) begin
        if (!rst_n_i) begin
            ...
            timeout_replay_o <= 1'b0;
            occupancy_q <= {(SLOT_BITS+1){1'b0}};
            time_q <= 16'd0;
            scan_slot_q <= {SLOT_BITS{1'b0}};
            for (reset_index = 0; reset_index < SLOTS; reset_index = reset_index + 1) begin
                ...
                last_send_time_q[reset_index] <= 16'd0;
            end
            ...
            if (store_fire && (store_active_q || store_start_i)) begin
                ...
                if (store_last_i) begin
                    ...
                    last_send_time_q[store_active_q ? store_slot_q : store_slot] <= time_q;
                    store_active_q <= 1'b0;
                    store_flit_q <= 4'd0;
                end else begin
                    store_active_q <= 1'b1;
            ...
            if (nack_valid_i && nack_match && !replay_active_q &&
                !replay_output_valid_q) begin
                ...
                if (retry_count_q[scan_slot_q] == 3'd7) begin
                    retry_exhausted_o <= 1'b1;
                end else begin
                    retry_count_q[scan_slot_q] <= retry_count_q[scan_slot_q] + 1'b1;
                    replay_active_q <= 1'b1;
                    replay_slot_q <= scan_slot_q;
                    replay_flit_q <= 4'd0;
                    timeout_replay_o <= 1'b1;
                end
            end
            ...
            if (replay_output_valid_q && replay_ready_i && replay_output_last_q)
                last_send_time_q[replay_slot_q] <= time_q;
        end
    end
endmodule
