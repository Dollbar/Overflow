...
module kdlink_v2_reverse_ctrl (
    ...
    kdlink_v2_reverse_codec u_codec (
    ...
    assign body_version_good = body_version == `KDL2_SCHEMA_VERSION;
    assign body_epoch_good = body_link_epoch == link_epoch_i;
    assign body_identity_good = body_dst_node == local_node_i;
    assign body_type_good = body_message_type <= `KDL2_REVERSE_TYPE_LANE_STATUS;
    ...
    always @(posedge clk_i or negedge rst_n_i) begin
        if (!rst_n_i) begin
            ...
            if (body_accept) begin
                ...
                if (body_message_type <= `KDL2_REVERSE_TYPE_NACK) begin
                    credit_total_q[body_vc] <= body_credit_total;
                    credit_update_valid_o <= 1'b1;
                end
                if (body_message_type == `KDL2_REVERSE_TYPE_ACK) ack_valid_o <= 1'b1;
                if (body_message_type == `KDL2_REVERSE_TYPE_NACK) nack_valid_o <= 1'b1;
                if (body_message_type == `KDL2_REVERSE_TYPE_LINK_RESET) begin
                    link_reset_valid_o <= 1'b1;
                    link_up_o <= 1'b0;
                    for (vc_index = 0; vc_index < 8; vc_index = vc_index + 1) credit_total_q[vc_index] <= 16'd0;
                end
                if (body_message_type == `KDL2_REVERSE_TYPE_INIT_ACK) begin
                    init_ack_valid_o <= 1'b1;
                    link_up_o <= 1'b1;
                end
                if (body_message_type == `KDL2_REVERSE_TYPE_KEEPALIVE_ACK) keepalive_ack_valid_o <= 1'b1;
                if (body_message_type == `KDL2_REVERSE_TYPE_LANE_STATUS) lane_status_valid_o <= 1'b1;
            end
        end
    end
endmodule
