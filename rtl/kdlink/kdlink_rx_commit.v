...
module kdlink_rx_commit #(
    ...
    input wire link_reinitialize_i,
        ...
        ((header_retry && (header_vc == `KDL_VC_ROLE_REPLAY)) ||
         (!header_retry &&
          (((header_message_type == `KDL_MESSAGE_TYPE_DATA) &&
            (header_vc <= `KDL_VC_ROLE_POINT_TO_POINT)) ||
           ((header_message_type >= `KDL_MESSAGE_TYPE_COLL_SETUP) &&
            (header_message_type <= `KDL_MESSAGE_TYPE_COLL_ABORT) &&
    ...
    always @(posedge clk_i or negedge rst_n_i) begin
        if (!rst_n_i) begin
            ...
            for (reset_index = 0; reset_index < 8; reset_index = reset_index + 1)
                credit_total_q[reset_index] <= INITIAL_CREDIT_TOTAL;
        ...
        end else if (link_reinitialize_i) begin
            head_q <= {CONTEXT_BITS{1'b0}};
            tail_q <= {CONTEXT_BITS{1'b0}};
            context_count_q <= {(CONTEXT_BITS+1){1'b0}};
            rx_active_q <= 1'b0;
            ...
            for (reset_index = 0; reset_index < 8; reset_index = reset_index + 1)
                credit_total_q[reset_index] <= INITIAL_CREDIT_TOTAL;
            for (reset_index = 0; reset_index < 512; reset_index = reset_index + 1)
                history_valid_q[reset_index] <= 1'b0;
        end else begin
            duplicate_o <= 1'b0;
