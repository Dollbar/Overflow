...
module kdlink_header_checker ( // 定义 KDLink header 合法性检查器
    ...
    always @(*) begin // 组合检查全部冻结协议约束
        ...
        if (header_i[3:0] != `KDL_SCHEMA_VERSION) error_o[0] = 1'b1; // 检查协议版本
        if (message_type > `KDL_MESSAGE_TYPE_FAULT) error_o[1] = 1'b1; // 检查消息类型范围
        ...
        if ((retry && (vc != `KDL_VC_ROLE_REPLAY)) || (!retry && (((message_type == `KDL_MESSAGE_TYPE_DATA) && (vc > `KDL_VC_ROLE_POINT_TO_POINT)) || ((message_type >= `KDL_MESSAGE_TYPE_COLL_SETUP) && (message_type <= `KDL_MESSAGE_TYPE_COLL_ABORT) && (vc != `KDL_VC_ROLE_CONTROL)) || ((message_type >= `KDL_MESSAGE_TYPE_KEEPALIVE) && (vc != `KDL_VC_ROLE_MANAGEMENT))))) error_o[7] = 1'b1; // 检查原始 message VC 和统一 replay VC 映射
