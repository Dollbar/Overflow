`timescale 1ns/1ps // 与共享formatter同一仿真时间单位。
// 普通未压缩Write/WriteFull：保存完整事务并按相对Beat序列交付Data与区域BE。
`default_nettype none // 禁止隐式网络掩盖握手连接错误
module endpoint_write_originator #(parameter integer CANCEL_ENABLE=0,NATIVE_FIELDS_ENABLE=0,MESSAGE_ENABLE=0)( // Write发起保持模块：单事务有序交付而不分配独立Tag表
 ...
 input wire i_cancel, // 显式同期TX清空确认取消本地holding，不触碰Tag或ledger。
 input wire [1:0] i_request_vc,input wire i_request_tl_pool // TL账户独立于原生信用账户，仅原生字段模式解释。
,input wire i_request_is_message,input wire [1:0] i_request_num_beats // Message NUM明确描述输入Data数量，不借用LEN。
); // 结束Write整事务与TL序列化接口
wire is_message=(MESSAGE_ENABLE!=0)&&i_request_is_message; // 默认忽略新增未连接输入。
wire cancel_active; // 仅明确启用时解释追加取消端口。
assign cancel_active=(CANCEL_ENABLE!=0)&&i_cancel; // 默认旧实例忽略未连接追加端口。
...
assign beats=is_message?({1'b0,i_request_num_beats}+3'd1):beat_sum[8:6]; // 合法区域内只允许一至四个相对Beat
...
always @* begin // 组合产生范围掩码，不保留上一候选的BE状态
 ...
 for(byte_index=32'd0;byte_index<32'd256;byte_index=byte_index+32'd1) // 有界展开区域内全部256个字节位
...
assign profile_legal=(i_request_address[1:0]==2'd0)&&(is_message?!i_request_full:((end_byte<=10'd256)&& // 要求DWORD对齐且整个请求不跨256字节区域
...
assign encoded={128'd0,4'h1,(is_message?6'h2a:(i_request_full?6'h29:6'h28)),((NATIVE_FIELDS_ENABLE!=0)?i_request_vc:2'd0),i_request_asi, // 未压缩自然128位字段上方补NOP并固定VC零
 i_request_tag,((NATIVE_FIELDS_ENABLE!=0)?i_request_tl_pool:1'b0),i_request_attr,i_request_length,i_request_metadata,i_request_address[56:2],i_local_id,i_request_dst,3'd0,num_beats}; // 保留属性地址和ID全位，CLOAD与CWAY共三位清零
...
assign header_fire=!cancel_active&&i_header_taken&&r_pending&&!r_sent&&(r_captured||capture_fire); // 每个Header仅允许一次实际发送事件
...
wire [1791:0] unused_shifted;wire [6:0] unused_beat_sum; // 高位移出范围与对齐余数不进入输出。
assign unused_shifted=shifted[2303:512];assign unused_beat_sum={beat_sum[9],beat_sum[5:0]}; // 显式标记自然截断边界。
...
assign o_request_ready=i_rstn&&!cancel_active&&!r_pending&&profile_legal; // 仅合法候选且holding空闲时接受新Write
assign o_pending=i_rstn&&r_pending; // 同步复位期间抑制此前事务有效
assign o_source_valid=i_rstn&&!cancel_active&&r_pending&&!r_captured; // Header被prepared捕获后不再重复提供
assign o_source_control=o_source_valid?r_control:256'd0; // 有效时输出保存Control，其他周期输出NOP
assign o_data_valid=(!i_rstn||cancel_active||!r_pending)?2'd0:((r_total-r_index)>=4'd2)?2'd2:(r_total>r_index)?2'd1:2'd0; // 按剩余数量提供最多两个半字，不凭Header已发送提前清空
...
assign o_done=i_rstn&&!cancel_active&&r_pending&&(r_captured||capture_fire)&&(r_sent||header_fire)&&(next_index==r_total); // Header捕获、真实发送及所有Data同时闭合才释放holding
assign o_error=i_rstn&&!cancel_active&&((i_request_valid&&!profile_legal)||(i_source_captured&&!o_source_valid)|| // 非法应用字段与不属于当前Header的捕获均报告诊断
...
always @(posedge i_clk)begin // 同一同步域原子更新事务与半字交付状态
 ...
 end else if(cancel_active)begin // 已授权TX清空仅撤本地holding，不清共享Tag或ledger。
  r_pending<=1'b0;r_captured<=1'b0;r_sent<=1'b0;r_control<=256'd0; // 取消剩余Header持有。
  r_payload<=2304'd0;r_index<=4'd0;r_total<=4'd0; // 取消剩余Data和BE，保持其他owner义务。
