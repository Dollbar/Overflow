...
module endpoint_receive_transactions #(parameter WRITE_ENABLE=0,FULL_READ_ENABLE=0,NATIVE_FIELDS_ENABLE=0,MESSAGE_ENABLE=0,parameter MESSAGE_RESPONSE_ENABLE=0)(
 input wire i_clk,i_rstn,input wire i_request_admission_enable,input wire [1:0] i_port,
 ...
 output wire o_idle, // 完整记录与所有部分请求/响应均已转交才空。
 output wire [9:0] o_response_src,output wire [1:0] o_response_vc,output wire o_response_pool,output wire [63:0] o_response_header, // 完整真实响应字段；SRC保留调试语义，不作为额外身份条件。
 output wire [127:0] o_request_header,output wire [1:0] o_request_port
 ,output wire o_request_reject_pulse,output wire [1:0] o_request_reject_port
);
reg [599:0] r_record; // 保持原退休记录观察点，两个展开模式均整字接纳
 generate if(WRITE_ENABLE==0&&FULL_READ_ENABLE==0)begin:read_only
 assign o_response_src=10'd0;assign o_response_vc=2'd0;assign o_response_pool=1'b0;assign o_response_header=64'd0;
...
reg [1:0] r_port;
reg [3:0] r_sector;
reg r_error,r_request_admitted;
...
wire [63:0] field64=shifted[63:0];
 assign o_request_header=o_request_valid?field128:128'd0;assign o_request_port=o_request_valid?r_port:2'd0;
...
wire assembler_idle; // 不能用ready推测部分响应队列为空。
...
assign o_idle=i_rstn&&!o_error&&(r_state==EMPTY)&&assembler_idle; // 原Read链包含真实assembler所有权。
assign o_read_ready=i_rstn&&!o_error&&(r_state==EMPTY);
assign o_request_valid=i_rstn&&!o_error&&request_here&&r_request_admitted;
assign o_request_reject_pulse=i_rstn&&!o_error&&request_here&&!r_request_admitted;
assign o_request_reject_port=o_request_reject_pulse?r_port:2'd0;
...
endpoint_response_assembler u_assembler(
 ...
 .o_response_data_error(o_response_data_error),.o_error(assembler_error),.o_idle(assembler_idle));
...
always @(posedge i_clk)begin
 if(!i_rstn)begin r_state<=EMPTY;r_record<=0;r_port<=0;r_sector<=0;r_error<=0;r_request_admitted<=1'b0;end
 else if(!o_error)begin
  case(r_state)
   EMPTY:if(i_read_valid&&o_read_ready)begin
    r_record<={i_read_releases,i_read_classes,i_read_msg,i_read_flit};r_port<=i_port;r_request_admitted<=i_request_admission_enable;r_state<=CHECK;
   end
   ...
   SCAN:begin
    ...
    else if((!request_here||!r_request_admitted||i_request_ready)&&(!response_here||header_ready))r_sector<=r_sector+4'd1;
   end
...
reg [2:0] state; reg [1:0] port; reg [3:0] sector; reg error_q,upper_done,record_admitted; // 准入在完整退休字接纳边界冻结。
...
reg [7:0] request_complete,response_complete,request_admitted; // 完整内容才可交付后端或Tag表；准入资格在Header入队时冻结。
...
wire field_write=f128[123:118]==6'h28||f128[123:118]==6'h29||((MESSAGE_ENABLE!=0)&&f128[123:118]==6'h2a);wire field_read_response=f64[37]; // 无Data字段不进入所有者队列
...
wire req_discard=request_count!=0&&request_complete[request_head]&&!request_admitted[request_head];
wire req_pop=(o_request_valid&&i_request_ready)||req_discard;
...
wire payload_event=i_rstn&&!error_q&&payload_state&&(payload_class==1||payload_class==2||((MESSAGE_RESPONSE_ENABLE!=0)&&payload_class==5)); // 普通Message/NOP不消费Data
...
wire data_message=(MESSAGE_ENABLE!=0)&&data_request[123:118]==6'h2a;
...
reg [3:0] response_poison[0:7]; // 和同一真实响应组装slot保存每Beat标记，不创建新owner。
wire response_half_consistent=!half_count[0]||(response_poison[data_slot][half_count[2:1]]==(payload_class==5)); // 即使直接600位边界也不得用不一致Poison半字完成。
wire payload_ok=owner_count!=0&&payload_port_ok&&((expect_be&&payload_class==2&&(data_message||((payload&~allowed_be)==0)))||(!expect_be&&((payload_class==1)||((MESSAGE_RESPONSE_ENABLE!=0)&&!data_is_write&&payload_class==5))&&(data_is_write||response_half_consistent))); // 非法BE或无归属Data不推进
...
assign o_idle=i_rstn&&!error_q&&(state==EMPTY)&&(request_count==0)&&(response_count==0)&&(owner_count==0); // 混合模式包含未齐套队列与Data所有者。
assign o_error=error_q;assign o_read_ready=i_rstn&&!error_q&&state==EMPTY; // 原600位所有权仅接纳一次
assign o_request_valid=i_rstn&&!error_q&&request_count!=0&&request_complete[request_head]&&request_admitted[request_head]; // drain后新Request完整消费并安全丢弃，已准入事务继续。
assign o_request_reject_pulse=i_rstn&&!error_q&&req_discard;
assign o_request_reject_port=o_request_reject_pulse?request_port[request_head]:2'd0;
assign o_request_header=req_out;assign o_request_port=o_request_valid?request_port[request_head]:2'd0;
...
assign o_response_header=rsp_out;assign o_response_src=rsp_out[35:26];assign o_response_vc=rsp_out[59:58];assign o_response_pool=rsp_out[46];
...
assign o_response_data=o_response_valid&&rsp_out[37]?response_data[response_head][response_beat*512+:512]:512'd0;assign o_response_data_error=(MESSAGE_RESPONSE_ENABLE!=0)&&o_response_valid&&rsp_out[37]&&response_poison[response_head][response_beat]; // 完整保护过的Poison交由原Tag结合保存的Message资格检查。
...
function request_legal; // 未压缩Read子集及完整普通Write几何校验
 ...
 request_legal=f[127:124]==1&&((NATIVE_FIELDS_ENABLE!=0)||(f[117:116]==0&&!f[102]))&&!f[4]&&
 (((MESSAGE_ENABLE!=0)&&f[123:118]==6'h2a)?1'b1:(write_cmd?((WRITE_ENABLE!=0)&&offset+size<=256&&beats>=1&&beats<=4&&{7'd0,f[1:0]}==beats-9'd1&&
...
function response_legal; // 写响应忽略OFFSET/LAST的无效取值，不强制建议值
 ...
 begin
 status_ok=((MESSAGE_RESPONSE_ENABLE!=0)&&(f[37]||f[41:38]!=4'hf))||f[41:38]==0||f[41:38]==3||((!f[37]||(FULL_READ_ENABLE!=0))&&(f[41:38]==2||f[41:38]==6||f[41:38]==8));
 unused_fields=^{f[57:47],f[35:16],f[13:0]}; // 响应SRC与SPARE不成为身份/合法性条件
 response_legal=f[63:60]==2&&((NATIVE_FIELDS_ENABLE!=0)||(f[59:58]==0&&!f[46]))&&status_ok&&f[15:14]==0&&
...
always @* begin
 bad=0;control_has_data=0;check_kind=0;check_word=0;probe=0;h=0;s=0; // 循环游标也完整赋值，综合不得把仿真迭代变量保存成锁存状态。
 for(h=0;h<2;h=h+1)begin
  ...
  case(check_kind)
   ...
   1,2:begin end
   5:if(MESSAGE_RESPONSE_ENABLE==0)bad=1; // 完整响应组装可以保存Poison，最终Tag决定该请求是否准许。
 ...
 if(lower_class==0)begin
  for(s=0;s<8;s=s+1)begin
   ...
   if(request_starts[s])begin
    ...
    if(probe[123:118]==6'h28||probe[123:118]==6'h29||((MESSAGE_ENABLE!=0)&&probe[123:118]==6'h2a))control_has_data=1;
   end
   if(response_starts[s])begin
  ...
  if(owner_count==0&&(upper_class==2||((upper_class==1||((MESSAGE_RESPONSE_ENABLE!=0)&&upper_class==5))&&!control_has_data)))bad=1; // 新所有者的第一个半字必须是Data
 end
end
always @(posedge i_clk)begin
 if(!i_rstn)begin
  state<=EMPTY;r_record<=0;port<=0;sector<=0;error_q<=0;upper_done<=0;record_admitted<=1'b0;
  request_head<=0;request_tail<=0;request_count<=0;request_complete<=0;request_admitted<=0;
  ...
  if(req_push)begin
   requests[request_tail]<=f128;request_port[request_tail]<=port;request_data[request_tail]<=0;request_be[request_tail]<=0;request_admitted[request_tail]<=record_admitted;
   request_complete[request_tail]<=!field_write;request_tail<=request_tail+1'b1;
  end
  if(rsp_push)begin
   responses[response_tail]<=f64;response_port[response_tail]<=port;response_data[response_tail]<=0;response_poison[response_tail]<=0;
  ...
  if(req_pop)begin request_complete[request_head]<=0;request_admitted[request_head]<=1'b0;request_head<=request_head+1'b1;end
  ...
  if(payload_event)begin
   if(!payload_ok)error_q<=1;
    ...
    if(data_is_write)begin
     ...
     if(MESSAGE_RESPONSE_ENABLE!=0)response_poison[data_slot][half_count[2:1]]<=(payload_class==5);
  ...
  case(state)
   EMPTY:if(i_read_valid&&o_read_ready)begin r_record<={i_read_releases,i_read_classes,i_read_msg,i_read_flit};port<=i_port;record_admitted<=i_request_admission_enable;state<=CHECK;upper_done<=0;end
   CHECK:if(bad)error_q<=1;else state<=LOWER;
   LOWER:if(lower_class==0)begin sector<=0;if(owner_count!=0&&(upper_class==1||upper_class==2||((MESSAGE_RESPONSE_ENABLE!=0)&&upper_class==5)))state<=EARLY;else state<=SCAN;end else state<=UPPER;
...
generate if((NATIVE_FIELDS_ENABLE!=0)&&((NATIVE_FIELDS_ENABLE!=1)||(WRITE_ENABLE!=1)||(FULL_READ_ENABLE!=1)))begin:invalid_native_config
 native_receive_requires_mixed_full_read Invalid_Config();
end endgenerate
generate if((MESSAGE_ENABLE!=0)&&((MESSAGE_ENABLE!=1)||(NATIVE_FIELDS_ENABLE!=1)||(WRITE_ENABLE!=1)||(FULL_READ_ENABLE!=1)))begin:invalid_message_config
 endpoint_message_requires_native_mixed Invalid_Config();
end endgenerate
generate if(MESSAGE_RESPONSE_ENABLE!=0&&((MESSAGE_RESPONSE_ENABLE!=1)||(MESSAGE_ENABLE!=1)||(NATIVE_FIELDS_ENABLE!=1)||(WRITE_ENABLE!=1)||(FULL_READ_ENABLE!=1)))begin:invalid_message_response_config
 endpoint_message_response_requires_native_mixed u_bad();
end endgenerate
endmodule
