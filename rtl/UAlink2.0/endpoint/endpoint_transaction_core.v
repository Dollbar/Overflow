...
assign o_error=i_rstn&&(!CONFIG_LEGAL||originator_error||receiver_error||completer_error||write_completer_error); // 配置诊断不伪装正常满槽背压
generate if(WRITE_ENABLE!=0) begin:mixed_originator
endpoint_request_formatter #(.CAPACITY(ORIGINATOR_CAPACITY_SAFE),.NUM_PORTS(1)) u_originator(
 ...
 .i_response_is_write(response_is_write),.o_complete_is_write(o_complete_is_write),
...
endpoint_receive_transactions #(.WRITE_ENABLE(WRITE_ENABLE)) u_receiver(
...
generate if(WRITE_ENABLE!=0) begin:mixed_completer
 ...
 wire dispatch_result=dispatch_issued&&(dispatch_write?
  ...
  (i_mem_result_valid&&o_mem_result_ready&&(i_mem_result_slot==dispatch_slot)&&((i_mem_result_status==4'd0)||(i_mem_result_status==4'd3))));
