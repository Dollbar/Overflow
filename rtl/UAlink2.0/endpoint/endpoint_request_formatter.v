...
module endpoint_request_formatter #(parameter integer CAPACITY=4,NUM_PORTS=1)( // 共享Read与Write发起模块：参数决定统一Tag容量和端口域
...
wire read_legal;wire [255:0] read_control; // 实际Read编码器给出完整字段及候选合法性
...
assign profile_legal=i_request_is_write?write_legal:(read_legal&&(i_request_asi==2'd0)&&(i_request_metadata==8'd0)); // Write检查完整几何；Read继续采用单64字节局部profile
...
endpoint_read_encode Read_Encode_Inst( // 实例化实际Read字段编码器，保持既有profile语义
 .i_valid(1'b1),.i_tag(i_request_tag),.i_src(i_local_id),.i_dst(i_request_dst),.i_address(i_request_address), // 候选编码保留Tag、地址与双侧物理ID
 .i_length(i_request_length),.i_attr(i_request_attr),.i_vc(2'd0),.i_pool(1'b0),.i_asi(2'd0),.i_metadata(8'd0), // Read固定VC信用域及当前支持的地址空间元数据
 .o_valid(read_legal),.o_error(),.o_control(read_control) // 合法性用于共享预约；编码错误等价于候选非法
); // 结束Read编码器连接
...
endpoint_tag_table #(.CAPACITY(CAPACITY),.NUM_PORTS(NUM_PORTS),.WRITE_ENABLE(1)) Tags_Inst( // 唯一共享Tag表显式开启Read与Write种类比较
