...
module endpoint_tag_table #(parameter integer CAPACITY=4, NUM_PORTS=1, WRITE_ENABLE=0, FULL_READ_ENABLE=0)( // Tag表模块：预约、实际发送、完整响应和应用退休四阶段所有权
...
reg r_is_write[0:CAPACITY-1]; // Read/Write共享同一完整(port,Tag)唯一域
...
assign read_full_legal=status_ordinary&&(i_response_offset<=r_read_num_beats[response_slot])&& // OFFSET相对整个Read请求
...
always @* begin // 避免逐字节动态写入整2048位寄存器形成冗余中间总线
 ...
 for(response_byte=0;response_byte<64;response_byte=response_byte+1)begin // 固定lane独立参与mask
  if(response_mask_shifted[response_byte]&&(i_response_status==0)) // 本地API策略，线上未选lane不要求零
...
assign response_legal=CONFIG_LEGAL&&response_found&&r_sent[response_slot]&&!r_done[response_slot]&& // 响应必须属于沿前已真实发送且尚未完成的预约
 (i_response_dst==i_local_id)&&(r_is_write[response_slot]==response_is_write)&& // 完整身份匹配后仍要求本地目的与请求种类一致
 !i_response_data_error&& // Poison不属于本阶段完整Read支持范围，非法输入不能改变所有权
 (response_is_write?((i_response_num_beats==0)&&status_ordinary): // Write忽略无效OFFSET与LAST
...
assign o_complete_data_valid=o_complete_valid&&(r_status[r_complete_slot]==4'd0)&&!r_is_write[r_complete_slot]; // 错误完成不提交成功数据
...
always @(posedge i_clk) begin // 单一同步域持有计数和完成仲裁
 ...
 always @(posedge i_clk)begin // 本地槽状态和结果数据采用相同时钟
  if(!i_rstn)begin // 同步取消该槽预约和残留结果
   ...
   r_is_write[slot]<=1'b0;r_port[slot]<=2'd0;r_tag[slot]<=11'd0;r_status[slot]<=4'd0;r_data[slot]<={RESULT_WIDTH{1'b0}}; // 清除旧时期身份和结果
  r_read_num_beats[slot]<=0;r_read_mask[slot]<=0;r_seen[slot]<=0;r_multi[slot]<=0; // 取消旧几何和部分收集
   ...
   if(allocate_fire&&(free_slot==SLOT_INDEX))begin // 预约只写入沿前空闲槽
    ...
    r_is_write[slot]<=allocate_is_write;r_port[slot]<=i_allocate_port;r_tag[slot]<=i_allocate_tag; // 完整身份在预约沿捕获
   ...
   r_read_num_beats[slot]<=((FULL_READ_ENABLE!=0)&&!allocate_is_write)?i_allocate_read_num_beats:2'd0; // 默认和Write不依赖新增输入
    ...
    r_seen[slot]<=0;r_multi[slot]<=0; // 新请求没有历史响应模式或Beat
   end // 结束本槽预约写入
   ...
   if(i_response_valid&&o_response_ready&&response_legal&&(response_slot==SLOT_INDEX))begin // 非法响应永不触发结果写入
    r_status[slot]<=i_response_status; // 合法响应始终与本事务已有状态一致
