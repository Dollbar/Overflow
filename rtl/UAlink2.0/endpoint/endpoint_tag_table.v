...
module endpoint_tag_table #(parameter integer CAPACITY=4, NUM_PORTS=1, WRITE_ENABLE=0)( // Tag表模块：预约、实际发送、完整响应和应用退休四阶段所有权
...
assign response_legal=CONFIG_LEGAL&&response_found&&r_sent[response_slot]&&!r_done[response_slot]&& // 响应必须属于沿前已真实发送且尚未完成的预约
 ...
 (i_response_num_beats==2'd0)&&!i_response_data_error&& // 当前普通响应长度为零且不允许DataError完成
 (response_is_write?((i_response_status==4'd0)||(i_response_status==4'd2)||(i_response_status==4'd3)||(i_response_status==4'd6)||(i_response_status==4'd8)): // Write保留五种普通远端完成状态
 (((i_response_status==4'd0)||(i_response_status==4'd3))&&(i_response_offset==2'd0)&&i_response_last)); // Write忽略无效OFFSET/LAST，Read仍要求完整单Beat
...
always @(posedge i_clk) begin // 单一同步域持有计数和完成仲裁
 ...
 always @(posedge i_clk)begin // 本地槽状态和结果数据采用相同时钟
  if(!i_rstn)begin // 同步取消该槽预约和残留结果
   ...
   r_is_write[slot]<=1'b0;r_port[slot]<=2'd0;r_tag[slot]<=11'd0;r_status[slot]<=4'd0;r_data[slot]<=512'd0; // 清除旧时期身份和结果
   ...
   if(i_response_valid&&o_response_ready&&response_legal&&(response_slot==SLOT_INDEX))begin // 非法响应永不触发结果写入
    ...
    r_data[slot]<=(!response_is_write&&(i_response_status==4'd0))?i_response_data:512'd0; // 错误响应虽完整接收但成功数据存储置零
   end // 结束完整响应捕获
