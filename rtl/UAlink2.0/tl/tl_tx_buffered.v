...
,parameter integer STOP_ENABLE=0,POISON_ENABLE=0,PACKET_BOUNDARY_ENABLE=0 // 默认关闭追加的Request隔离选择。
 ...
 output wire o_valid,output wire [511:0] o_flit,output wire [1:0] o_msg,output wire o_packet_sop,o_packet_eop, // 发给同一真实信用端口的完整候选
...
wire [1:0] poison0,poison1; // 标记由真实Data SRAM读取。
...
genvar lane;generate for(lane=0;lane<2;lane=lane+1)begin:gen_queues // 每个类别各有头部与数据容量，禁止相互借用
 ...
 tl_tx_data_fifo #(.BANK_DEPTH(BANK_DEPTH),.COUNT_WIDTH(DATA_COUNT_WIDTH),.POISON_ENABLE(POISON_ENABLE)) Data_Inst( // 两个真实SRAM bank允许每拍一或二半Flit
 ...
 .i_poison0(i_poison0[lane]),.i_poison1(i_poison1[lane]),.o_poison0(poison0[lane]),.o_poison1(poison1[lane]),
...
tl_tx_channels #(.WIDTH(WIDTH),.RAW_HEADERS(1),.STOP_ENABLE(STOP_ENABLE),.POISON_ENABLE(POISON_ENABLE),.PACKET_BOUNDARY_ENABLE(PACKET_BOUNDARY_ENABLE)) Channels_Inst( // 复用已验证独立准入、旧Data所有权和FC公平打包
 ...
 .i_poison0(poison0),.i_poison1(poison1),
 .i_data_valid(data_valid),.i_data0(data0),.i_data1(data1), // 缺数据时等待真实SRAM返回，禁止用输入旁路替代
 .i_fc_valid(i_fc_valid),.i_fc_flit(i_fc_flit),.i_fc_msg(i_fc_msg),.i_stop_request(i_stop_request), // 信用返回继续使用独立实际发布器
 .o_valid(o_valid),.o_flit(o_flit),.o_msg(o_msg),.o_packet_sop(o_packet_sop),.o_packet_eop(o_packet_eop),.o_header_taken(header_taken),.o_tags_taken(tags_taken),.o_data_taken(data_taken),.o_fc_taken(o_fc_taken), // 只有真实发送确认修改缓存消费状态
