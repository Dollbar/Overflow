...
genvar j;generate for(j=0;j<20;j=j+1)begin:extend_demand // 保留所有四位需求
 ...
 tl_credit_integration #(.WIDTH(WIDTH)) u_rx(i_clk,i_rstn,i_receive,i_send&&tx_valid&&!credit_free,i_auth,i_rx_flit,i_rx_msg,extended_demands,o_rx_allowed,o_rx_taken,o_rx_rejected,credit_allowed,unused_credit_taken,o_rx_lower,o_rx_upper,o_rx_pending,o_rx_be,o_requests_available,o_responses_available,o_fatal,o_pair_open,o_pair_poison,o_capacity,o_available,o_done,o_shared,o_init_repeat,o_init_conflict,o_rx_grants,o_rx_init,o_rx_runtime); // 真实FC与MSG进入同一ledger
