module tl_tx_packer_core #(parameter integer CANCEL_ENABLE=0,POISON_ENABLE=0,PACKET_BOUNDARY_ENABLE=0)( // tl_tx_packer_core模块：复用已计算资格，仅选择来源并打包
 input wire i_clk,i_rstn,i_taken, // 唯一上升沿时钟、同步低有效复位和真实发送确认
 input wire [6:0] i_pending,input wire i_auth, // 原始发送序列位置与认证状态
 input wire i_header_ready,i_nop_ready,i_has_data,input wire [7:0] i_header_tenure, // 已核对头部资格、catch NOP资格及真实tenure负载标记
...
,output wire o_held_header,output reg o_packet_sop,o_packet_eop // 候选随真实selection锁定的内部包边界。
,input wire i_cancel_offer // 仅撤销尚未被下游接纳的内部提议，不清存储责任。
,input wire i_poison0,i_poison1 // 已选原Data队首的标记。
); // 结束内部资格复用接口
...
assign o_held_header=(r_hold==SEL_HEADER); // 让调用者区分请求Header与不应撤销的FC/Response Data。
wire poison0=(POISON_ENABLE!=0)&&i_poison0,poison1=(POISON_ENABLE!=0)&&i_poison1; // 默认接口忽略未连接输入。
wire [255:0] data0=poison0?{i_data0[255:8],8'h20}:i_data0,data1=poison1?{i_data1[255:8],8'h20}:i_data1; // 仅低字节标准化为Poison类型。
wire fc_ready; // FC资格仍依赖真实序列及所选有序Data源
assign fc_ready=i_fc_valid&&((i_pending==7'd0)||((PACKET_BOUNDARY_ENABLE==0)&&(i_pending==7'd1)&&(i_data_valid>=2'd1)&&(i_fc_msg==2'd0))); // 自动边界模式先独立退休旧尾，禁止一个TL字跨业务包与FC包。
always @* begin // 根据真实序列与已锁定来源选择当前候选
 selection=SEL_NONE; // 默认没有可发送候选
 if(i_rstn)begin // 复位期间不对外提议或消费输入
  if((r_hold!=SEL_NONE)&&!((CANCEL_ENABLE!=0)&&i_cancel_offer))selection=r_hold; // 停顿保持已选来源，允许其他源到达而不改写输出
  else if(i_pending>7'd1)begin // Data序列内部禁止Control或FC插入
   if(i_data_valid>=2'd2)selection=SEL_DATA; // 两个有序半Flit同时有效才构造完整Data Flit
  end else if((PACKET_BOUNDARY_ENABLE!=0)&&(i_pending==7'd1)&&(i_data_valid>=2'd1))selection=SEL_TAIL; // 自动边界先闭合旧packet，避免同一TL字同时EOP和SOP。
  else if(fc_ready&&(!i_header_ready||r_prefer_fc))selection=SEL_FC; // 轮到FC或业务不可发时先发FC
...
always @* begin // 被选来源组合成完整Flit以及独立消费计数
 o_valid=1'b0;o_flit=512'd0;o_msg=2'd0;data_count=2'd0;tags_selected=1'b0;o_packet_sop=1'b1;o_packet_eop=1'b1; // 旧模式保持每word独立成帧。
 case(selection) // 按已确定的候选来源打包
  SEL_HEADER:begin // 新Control固定放在下半Flit
   o_valid=1'b1;o_flit[255:0]=i_header; // 只有实际taken才弹出头部
   if(i_pending==7'd1)begin o_flit[511:256]=data0;o_msg[1]=poison0;data_count=2'd1;end // 旧尾部交换到上半，不提前消费新数据
   else if(i_auth)begin o_flit[511:256]=i_tags;tags_selected=1'b1;end // 对应AuthTags与头部在同一个Flit
   else if(i_has_data)begin o_flit[511:256]=data0;o_msg[1]=poison0;data_count=2'd1;end // 无认证且有Data时上半消费第一条数据
  end // 结束新Control打包分支
  SEL_FC:begin // FC或完成消息来自真实发布器
   o_valid=1'b1;o_flit=i_fc_flit;o_msg=i_fc_msg; // 保持发布器原编码
   if(i_pending==7'd1)begin o_flit[511:256]=data0;if(POISON_ENABLE!=0)o_msg[1]=poison0;data_count=2'd1;end // 普通FC上半携带旧Data或BE尾部
  end // 结束FC打包分支
  SEL_DATA:begin o_valid=1'b1;o_flit={data1,data0};o_msg={poison1,poison0};data_count=2'd2;end // 有序两个Data或BE半Flit
  SEL_TAIL:begin o_valid=1'b1;o_flit={data0,256'd0};o_msg[1]=poison0;data_count=2'd1;end // 下半NOP允许旧尾部独立退休
  SEL_NOP:begin o_valid=1'b1;end // 完整零NOP推进catch预算且不确认任何输入队首
  default:begin end // 没有合法候选时保留所有零默认输出
 endcase // 结束候选负载打包选择
 if(PACKET_BOUNDARY_ENABLE!=0)begin // 自动模式只使用已选来源和同一tenure owner，不从输出格式反推。
  case(selection)
   SEL_HEADER:begin o_packet_sop=1'b1;o_packet_eop=(i_header_tenure<=8'd1);end // Control同拍最多消费第一条Data。
   SEL_DATA:begin o_packet_sop=1'b0;o_packet_eop=(i_pending<=7'd2);end // 两个有序半Flit覆盖余量时闭包。
   SEL_TAIL:begin o_packet_sop=1'b0;o_packet_eop=1'b1;end // 唯一旧尾严格闭包。
   SEL_FC,SEL_NOP:begin o_packet_sop=1'b1;o_packet_eop=1'b1;end // FC与catch NOP保持独立帧，不与业务混账。
   default:begin o_packet_sop=1'b0;o_packet_eop=1'b0;end // 无候选不声明虚假边界。
  endcase
 end
end // 结束负载和消费计数组合逻辑
