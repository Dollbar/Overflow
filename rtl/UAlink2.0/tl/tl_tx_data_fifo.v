...
,parameter integer POISON_ENABLE=0 // 默认保持原256位存储。
...
,input wire i_poison0,i_poison1,output wire o_poison0,o_poison1 // 每半字独立标记随同真实入队/出队。
); // 结束有序发送缓存端口
localparam integer STORAGE_WIDTH=256+((POISON_ENABLE!=0)?8:0); // 无额外队列。
wire [263:0] packed0={((POISON_ENABLE!=0)&&i_poison0),7'd0,i_data0},packed1={((POISON_ENABLE!=0)&&i_poison1),7'd0,i_data1}; // 同一SRAM字原子保存。
...
wire [STORAGE_WIDTH-1:0] read_data0,read_data1;wire [COUNT_WIDTH-1:0] count0,count1; // 实际bank数据及全生命周期容量
wire [7:0] unused_write_metadata0=packed0[263:256],unused_write_metadata1=packed1[263:256]; // 默认256模式仅丢弃禁用元数据。
wire [STORAGE_WIDTH-1:0] unused_read_padding0=read_data0>>256,unused_read_padding1=read_data1>>256; // 未用填充高位不参与任何判断。
...
assign o_data0=(o_valid_count!=2'd0)?(r_read_bank?read_data1[255:0]:read_data0[255:0]):256'd0; // 队首只由正确bank已完成读缓存提供
assign o_data1=(o_valid_count==2'd2)?(r_read_bank?read_data0[255:0]:read_data1[255:0]):256'd0; // 第二项按逻辑顺序选择另一个bank
assign o_poison0=(POISON_ENABLE!=0)&&(o_valid_count!=0)&&(r_read_bank?read_data1[STORAGE_WIDTH-1]:read_data0[STORAGE_WIDTH-1]); // 只标记真实可见首项。
assign o_poison1=(POISON_ENABLE!=0)&&(o_valid_count==2)&&(r_read_bank?read_data0[STORAGE_WIDTH-1]:read_data1[STORAGE_WIDTH-1]); // 第二项使用同一读条带。
...
upli_receive_storage #(.C_DEPTH(BANK_DEPTH),.C_DATA_WIDTH(STORAGE_WIDTH),.C_COUNT_WIDTH(COUNT_WIDTH)) Bank0_Inst( // 复用已验证注册读FIFO控制及获授权SRAM映射
 .i_clk(i_clk),.i_rstn(i_rstn),.i_write_valid(write_enable[0]),.i_write_data(r_write_bank?packed1[STORAGE_WIDTH-1:0]:packed0[STORAGE_WIDTH-1:0]),.o_write_ready(write_ready[0]), // 第一bank写入对应条带的真实半Flit
 .i_read_ready(read_enable[0]),.o_read_valid(read_valid[0]),.o_read_data(read_data0),.o_count(count0) // 缓存与在途读仍计入原始容量
); // 结束第一bank实例
upli_receive_storage #(.C_DEPTH(BANK_DEPTH),.C_DATA_WIDTH(STORAGE_WIDTH),.C_COUNT_WIDTH(COUNT_WIDTH)) Bank1_Inst( // 第二bank与第一bank使用同一个输入时钟
 .i_clk(i_clk),.i_rstn(i_rstn),.i_write_valid(write_enable[1]),.i_write_data(r_write_bank?packed0[STORAGE_WIDTH-1:0]:packed1[STORAGE_WIDTH-1:0]),.o_write_ready(write_ready[1]), // 第二bank保持奇偶交替的数据顺序
