module tl_credit_integration #(parameter WIDTH=16)( // 实际Flit信用集成，WIDTH至少8
...
wire joint_commit=!o_fatal&&(!i_receive||o_rx_allowed); // 无组合反馈的最终账本提交
 tl_full_flit u_full( // 已验证完整分类/预算/内容
 .i_clk(i_clk),.i_rstn(i_rstn),.i_commit(i_receive&&!o_fatal&&events_valid&&!ledger_error),.i_auth(i_auth), // 信用失败阻止接收状态更新
 .i_flit(i_flit),.i_msg(i_msg),.o_allowed(full_allowed),.o_taken(unused_full_taken),.o_rejected(unused_full_rejected), // 实际Flit接口
 .o_lower(lower),.o_upper(upper),.o_pending(o_pending),.o_be(o_be), // 分类状态
 .o_requests_available(o_requests_available),.o_responses_available(o_responses_available), // 打包预算
 .o_fatal(full_fatal),.o_pair_open(o_pair_open),.o_pair_poison(o_pair_poison) // 内容与停止状态
 ); // 接收器连接结束
 ...
 tl_credit_ledger #(.WIDTH(WIDTH)) u_ledger(i_clk,i_rstn,joint_commit,i_receive,i_send,|init,mode,extended_grants,i_demands,ledger_allowed,o_tx_taken,ledger_error,o_capacity,o_available,o_done,o_shared); // 候选评估与提交分离
