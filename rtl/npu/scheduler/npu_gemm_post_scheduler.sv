...
module npu_gemm_post_scheduler #(
    ...
    input  logic external_completion_valid_i,
    output logic external_completion_ready_o,
    input  logic [npu_scheduler_pkg::NPU_TAG_WIDTH-1:0]
                 external_completion_tag_i,
    input  logic external_completion_success_i,
    ...
    npu_scheduler_pkg::npu_post_route_e lane_route [0:RESULT_CHANNELS-1];
    logic [CONTEXTS*6-1:0] context_increment;
    logic [CONTEXTS-1:0] context_dispatch_complete;
    logic external_completion_index_in_range;
    logic [CONTEXT_INDEX_WIDTH-1:0] external_completion_index;
    logic external_completion_match;
    logic external_completion_fire;
    ...
    always_comb begin
        ...
        for (integer lane = 0; lane < RESULT_CHANNELS; lane++) begin
            ...
            lane_route[lane] = lane_command.route;
            lane_context_index[
    ...
    // Keep ready/fire accounting separate from routed valid and payload. The
    // split makes each ready/valid dependency explicit and prevents a
    // destination arbiter from forming a false combinational valid loop.
    always_comb begin
        gemm_ready_o = '0;
        lane_fire = '0;
        context_increment = '0;
        ...
        for (integer lane = 0; lane < RESULT_CHANNELS; lane++) begin
            if (lane_match[lane]) begin
                unique case (lane_route[lane])
                    npu_scheduler_pkg::NPU_POST_EXTERNAL:
                        gemm_ready_o[lane] = !rst_i && !clear_i &&
                            external_ready_i[lane];
                    npu_scheduler_pkg::NPU_POST_VECTOR:
                        gemm_ready_o[lane] = !rst_i && !clear_i &&
                            vector_ready_i[lane];
                    npu_scheduler_pkg::NPU_POST_GEMM:
                        gemm_ready_o[lane] = !rst_i && !clear_i &&
                            feedback_ready_i[lane] &&
                            completion_slot_available;
                    default: gemm_ready_o[lane] = 1'b0;
                endcase
            end
    ...
    always_comb begin
        external_completion_index_in_range =
            ({1'b0, external_completion_tag_i[3:0]} < 5'(CONTEXTS));
        external_completion_index = CONTEXT_INDEX_WIDTH'(
            external_completion_tag_i[3:0]
        );
        external_completion_match = external_completion_index_in_range &&
            context_valid_q[external_completion_index] &&
            dispatch_done_q[external_completion_index] &&
            (context_command_mem[external_completion_index].tag ==
             external_completion_tag_i) &&
            (context_command_mem[external_completion_index].route ==
             npu_scheduler_pkg::NPU_POST_EXTERNAL);
        external_completion_ready_o = !rst_i && !clear_i &&
            external_completion_match && completion_slot_available;
        external_completion_fire = external_completion_valid_i &&
            external_completion_ready_o;
        ...
        vector_completion_match = vector_completion_index_in_range &&
            ...
            (context_command_mem[vector_completion_index].route ==
             npu_scheduler_pkg::NPU_POST_VECTOR) &&
            (context_command_mem[vector_completion_index].
                 vector_result_route ==
        ...
        feedback_completion_match = feedback_completion_index_in_range &&
            ...
            ((context_command_mem[feedback_completion_index].route ==
             ...
             ((context_command_mem[feedback_completion_index].route ==
               npu_scheduler_pkg::NPU_POST_VECTOR) &&
              (context_command_mem[feedback_completion_index].
                   vector_result_route ==
        ...
        feedback_completion_ready_o = !rst_i && !clear_i &&
            ...
            !external_completion_fire && !vector_completion_fire;
    ...
    always_ff @(posedge clk_i) begin
        if (rst_i || clear_i) begin
            ...
            if (external_completion_valid_i &&
                !external_completion_match) begin
                protocol_error_o <= 1'b1;
            end
            if (external_completion_fire) begin
                context_valid_q[external_completion_index] <= 1'b0;
                dispatch_done_q[external_completion_index] <= 1'b0;
                completion_valid_q <= 1'b1;
                completion_tag_q <= external_completion_tag_i;
                completion_success_q <= external_completion_success_i;
                if (!external_completion_success_i) begin
                    protocol_error_o <= 1'b1;
                end
            end
