...
module npu_input_scheduler #(
    ...
    logic [4:0] slot_vectors_per_row [0:TASK_SLOTS-1];
    ...
    logic incoming_descriptor_legal;
    npu_scheduler_pkg::npu_task_status_code_e incoming_error_code;
    ...
    function automatic logic mxfp_format_supported(
        input mxfp_pkg::mxfp_format_e format
    );
        begin
            mxfp_format_supported =
                (format == mxfp_pkg::MXFP4_E2M1) ||
                (format == mxfp_pkg::MXFP8_E4M3);
        end
    endfunction
                ...
                npu_scheduler_pkg::NPU_TASK_ERROR_OPERATION;
                     ...
                     ((incoming_descriptor.post_route ==
                      ...
                      (!mxfp_format_supported(
                       ...
                       !mxfp_format_supported(
                           incoming_descriptor.vector_c_format))) ||
                     ((incoming_descriptor.output_format ==
                       npu_scheduler_pkg::NPU_OUTPUT_MX) &&
                      !mxfp_format_supported(
                           incoming_descriptor.output_mx_format))) begin
            incoming_error_code =
                npu_scheduler_pkg::NPU_TASK_ERROR_FORMAT;
        end
        incoming_descriptor_legal =
            (incoming_error_code == npu_scheduler_pkg::NPU_TASK_STATUS_OK);
    end
    ...
    always_comb begin
        ...
        task_ready_o = !rst_i && !clear_i &&
            ((!task_valid_i && (free_task_valid || task_consume)) ||
             (task_valid_i && incoming_descriptor_legal &&
              (free_task_valid || task_consume)) ||
             (task_valid_i && !incoming_descriptor_legal &&
              status_fifo_input_ready && !completion_valid_i));
        task_accept = task_valid_i && task_ready_o;
        task_enqueue = task_accept && incoming_descriptor_legal;
        task_reject = task_accept && !incoming_descriptor_legal;
    end
    ...
    always_comb begin
        ...
        for (integer slot = 0; slot < TASK_SLOTS; slot++) begin
            ...
            slot_vectors_per_row[slot] = 5'(
                (task_mem[slot].matrix_size + 16'd15) >> 4);
    ...
    always_comb begin
        ...
        vector_command_input.control.output_format =
            (selected_descriptor.vector_result_route ==
             npu_scheduler_pkg::NPU_VECTOR_TO_FEEDBACK) ?
            vector_pkg::EPILOGUE_OUT_FP32 :
            vector_pkg::epilogue_output_format_e'(
        ...
        vector_command_input.operand_b_format =
            selected_descriptor.vector_b_format;
        ...
        result_command_input.vectors_per_row = selected_vectors_per_row;
        result_command_input.output_format = selected_descriptor.output_format;
        result_command_input.output_mx_format =
            selected_descriptor.output_mx_format;
        result_command_input.vector_result_route =
            selected_descriptor.vector_result_route;
        ...
        post_command_input.vector_control.output_format =
            (selected_descriptor.vector_result_route ==
             npu_scheduler_pkg::NPU_VECTOR_TO_FEEDBACK) ?
            vector_pkg::EPILOGUE_OUT_FP32 :
            vector_pkg::epilogue_output_format_e'(
                selected_descriptor.output_format);
        post_command_input.vector_control.mx_format =
            selected_descriptor.output_mx_format;
        post_command_input.operand_b_buffer_id =
            selected_descriptor.vector_b_buffer_id;
        post_command_input.operand_b_base_offset =
            selected_descriptor.vector_b_base_offset;
        post_command_input.operand_b_format =
            selected_descriptor.vector_b_format;
        post_command_input.operand_c_buffer_id =
            selected_descriptor.vector_c_buffer_id;
        post_command_input.operand_c_base_offset =
            selected_descriptor.vector_c_base_offset;
        post_command_input.operand_c_format =
            selected_descriptor.vector_c_format;
        ...
        post_command_input.output_mx_format =
            selected_descriptor.output_mx_format;
    ...
    always_comb begin
        status_input = '0;
        if (completion_valid_i && completion_context_match) begin
            status_input.job_id =
        ...
        end else if (completion_valid_i) begin
            ...
            status_input.job_id = incoming_descriptor.job_id;
            status_input.tag = '0;
            status_input.success = 1'b0;
            status_input.code = incoming_error_code;
        end
        status_fifo_input_valid = completion_valid_i || task_reject;
    ...
    initial begin
        ...
        assert ((ARRAY_DIM > 0) && (ARRAY_DIM <= 16))
            else $error("ARRAY_DIM must be in 1..16");
    end

    wire _unused_scheduler_fields = &{1'b0, selected_descriptor.version,
        selected_descriptor.operation,
        selected_descriptor.wait_event_valid,
        selected_descriptor.wait_event_id, incoming_descriptor};
