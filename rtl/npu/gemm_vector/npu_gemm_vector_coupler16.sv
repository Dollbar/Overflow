...
module npu_gemm_vector_coupler16 #(
    ...
    input  logic [CHANNELS*512-1:0] operand_b_read_data_i,
    ...
    input  logic [CHANNELS*512-1:0] operand_c_read_data_i,
            ...
            always_comb begin
                ...
                backend_response_observation[decode_lane] = ^{
                    ...
                    backend_response_decoded[decode_lane].control.
                        operand_b_source,
                    backend_response_decoded[decode_lane].control.rounding,
                    ...
                    backend_response_decoded[decode_lane].control.fp8_format,
    ...
    always_comb begin
        ...
        for (integer lane = 0; lane < CHANNELS; lane++) begin
            ...
            vector_offset = ((32'(lane_fifo_beat[lane].row[3:0]) *
                             ...
                             32'(lane_fifo_beat[lane].segment)) << 6;
            ...
            ] = lane_fifo_command[lane].operand_b_base_offset + vector_offset;
            ...
            operand_c_read_offset_o[
                lane*npu_scheduler_pkg::NPU_BUFFER_OFFSET_WIDTH +:
                npu_scheduler_pkg::NPU_BUFFER_OFFSET_WIDTH
            ] = lane_fifo_command[lane].operand_c_base_offset + vector_offset;
        end
    end
        ...
        for (integer lane = 0; lane < CHANNELS; lane++) begin
            ...
            output_index_in_range =
                ({1'b0, backend_response_decoded[lane].control.tag[3:0]} <
                 5'(CONTEXTS));
            output_context_match = output_index_in_range &&
