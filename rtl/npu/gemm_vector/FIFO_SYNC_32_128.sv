...
module FIFO_SYNC_32_128 ( // 使用一组32乘128真双端口SRAM构造同步FIFO
    ...
    input  logic [127:0] wr_data_i, // 提供待入队的128位数据
    input  logic         rd_ready_i, // 表示读侧愿意接受当前队首数据
    output logic         rd_valid_o, // 表示当前队首数据有效
    output logic [127:0] rd_data_o, // 输出保持到握手完成的128位队首数据
    ...
    logic [127:0] ram_read_data; // 接收SRAM读端口的同步返回数据
    ...
    assign rd_data_o = rd_valid_q ? ram_read_data : 128'd0; // 无有效队首时输出确定的零值
    assign read_fire = rd_valid_q && rd_ready_i; // 有效队首被读侧接受时完成出队
    // 满边界保守地等待计数下降后再接收，切断read_fire反馈到写侧ready的长组合环路。
    assign wr_ready_o = !flush_active && !full_o;
    ...
    SRAM_32_128 u_storage ( // 例化一组TSMC28真双端口128位SRAM
        ...
        .b_wdata_i  (128'd0), // 读专用B口的写数据固定为零
...
endmodule // 结束FIFO_SYNC_32_128模块
