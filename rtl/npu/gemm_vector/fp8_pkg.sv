...
package fp8_pkg; // 定义FP8格式、舍入模式与统一解码结果类型

typedef enum int unsigned { // 定义可用于常量切片的FP8字段宽度集合
    E4M3_EXP_WIDTH = 4, // 定义E4M3指数域宽度为四位
    E4M3_FRAC_WIDTH = 3, // 定义E4M3小数域宽度为三位
    E5M2_EXP_WIDTH = 5, // 定义E5M2指数域宽度为五位
    E5M2_FRAC_WIDTH = 2 // 定义E5M2小数域宽度为两位
} fp8_field_width_e; // 结束FP8字段宽度常量集合定义

typedef enum logic signed [7:0] { // 定义两种FP8格式的八位有符号指数偏置
    E4M3_BIAS = 8'sd7, // 定义E4M3指数偏置为七
    E5M2_BIAS = 8'sd15 // 定义E5M2指数偏置为十五
} fp8_bias_e; // 结束FP8指数偏置常量集合定义

typedef enum logic { // 定义运行时选择的FP8编码格式枚举
    FP8_E4M3 = 1'b0, // 选择无Infinity且仅尾数全一为NaN的OCP E4M3FN格式
    FP8_E5M2 = 1'b1 // 选择一位符号、五位指数和两位小数格式
} fp8_format_e; // 结束FP8编码格式枚举定义
