...
module vector_reduce16 #(
    ...
    localparam int unsigned CONTROL_WIDTH = 28;
    localparam int unsigned CONTROL_OPERATION_BIT = 2;
    localparam int unsigned CONTROL_LANE_MASK_LSB = 12;
    ...
    fp8_pkg::fp8_rounding_e rounding_l0_comb;
    fp8_pkg::fp8_rounding_e rounding_l1_comb;
    fp8_pkg::fp8_rounding_e rounding_l2_comb;
    fp8_pkg::fp8_rounding_e rounding_l3_comb;
    ...
    always_comb begin
        rounding_l0_comb = fp8_pkg::fp8_rounding_e'(control_delay_q[0][1:0]);
        rounding_l1_comb = fp8_pkg::fp8_rounding_e'(control_delay_q[6][1:0]);
        rounding_l2_comb = fp8_pkg::fp8_rounding_e'(control_delay_q[12][1:0]);
        rounding_l3_comb = fp8_pkg::fp8_rounding_e'(control_delay_q[18][1:0]);
    ...
    generate
        for (genvar pair = 0; pair < 8; pair++) begin : g_sum_l1
            fp32_add_pipeline #(.FTZ(FTZ)) u_add (
                ...
                .rounding_i(rounding_l0_comb), .valid_o(sum_valid_l1[pair]),
        ...
        for (genvar pair = 0; pair < 4; pair++) begin : g_sum_l2
            fp32_add_pipeline #(.FTZ(FTZ)) u_add (
                ...
                .rounding_i(rounding_l1_comb), .valid_o(sum_valid_l2[pair]),
        ...
        for (genvar pair = 0; pair < 2; pair++) begin : g_sum_l3
            fp32_add_pipeline #(.FTZ(FTZ)) u_add (
                ...
                .rounding_i(rounding_l2_comb), .valid_o(sum_valid_l3[pair]),
    ...
    fp32_add_pipeline #(.FTZ(FTZ)) u_sum_l4 (
        ...
        .rounding_i(rounding_l3_comb), .valid_o(sum_valid_l4),
