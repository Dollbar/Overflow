...
// Exact cross-K accumulator for one 16-column Tile result row. Each accepted
// K=32 MX block contributes a signed 70-bit Q32 partial. This signed 85-bit
// accumulator performs no intermediate rounding. The compiler guarantees one
// common combined scale exponent per output dot product.
module tile_fp32_k_accumulator #(
    parameter bit FTZ = 1'b0,
    parameter bit STREAM_METADATA_MODE = 1'b0,
    ...
    localparam int unsigned CONVERT_META_DEPTH = 2;
    localparam int unsigned REQUIRED_ACCUM_WIDTH = 70 + 15;
    ...
    logic [7:0] partial_stage_tag_q;
    logic signed [69:0] partial_stage_exact_q [0:15];
    ...
    logic signed [9:0] partial_stage_scale_q [0:15];
    ...
    logic signed [9:0] merge_scale_q [0:15];
    ...
    logic [3:0] cpa_stage_row_q;
    logic [7:0] cpa_stage_tag_q;
    ...
    logic signed [9:0] cpa_stage_scale_q [0:15];

    logic final_valid_q;
    logic final_last_q;
    logic [3:0] final_row_q;
    logic [7:0] final_tag_q;
    ...
    logic signed [9:0] final_scale_q [0:15];
    ...
    logic [3:0] convert_meta_row_q [0:CONVERT_META_DEPTH-1];
    logic [7:0] convert_meta_tag_q [0:CONVERT_META_DEPTH-1];
    logic converter_launch;
    logic converter_return;
    logic converter_return_last;
    logic [3:0] converter_return_row;
    logic [7:0] converter_return_tag;
    ...
    logic [3:0] incoming_row;

    assign command_active = STREAM_METADATA_MODE ? 1'b1 :
                            (command_level_q != '0);
    ...
    assign command_push = !STREAM_METADATA_MODE && start_i && command_has_space;
    assign incoming_row = STREAM_METADATA_MODE ? stream_row_i :
                          row_index_q[3:0];
    assign first_block = STREAM_METADATA_MODE ? stream_first_i : first_block_q;
    assign final_block = STREAM_METADATA_MODE ? stream_last_i : final_block_q;
    assign final_row = STREAM_METADATA_MODE ? (stream_row_i == 4'd15) :
                       ((row_index_q + 5'd1) == command_rows);
    ...
    assign command_pop = !STREAM_METADATA_MODE && partial_fire &&
                         final_block && final_row;
    assign start_ready_o = STREAM_METADATA_MODE ? !local_control_flush[0] :
        (!local_control_flush[0] && command_has_space);
    ...
    generate
        ...
        (* keep = "true", dont_touch = "true" *)
        tile_control_inverter u_command_push_root_inverter (
            .data_i (command_push),
            .data_o (command_push_n)
        );

        (* keep = "true", dont_touch = "true" *)
        tile_control_inverter u_command_rows_push_inverter (
            .data_i (command_push_n),
            .data_o (command_rows_push)
        );

        (* keep = "true", dont_touch = "true" *)
        tile_control_inverter u_command_k_blocks_push_inverter (
            .data_i (command_push_n),
    ...
    assign state_mem_read_addr = {1'b0, incoming_row};
    assign state_mem_read = partial_fire && !first_block &&
        !(partial_stage_valid_q &&
          (partial_stage_row_q == incoming_row)) &&
        !(merge_valid_q && !merge_final_q &&
          (merge_row_q == incoming_row));
    ...
    generate
        ...
        for (genvar lane = 0; lane < 16; lane = lane + 1) begin : gen_exact_merge
            ...
            assign partial_extended =
                {{(ACCUM_WIDTH-70){partial_stage_exact_q[lane][69]}},
            ...
            mxfp_block_sum_to_fp32 #(
                .FTZ         (FTZ),
                .EXACT_WIDTH (ACCUM_WIDTH)
            ) u_final_converter (
                .clk_i       (clk_i),
                ...
                .exact_i     (final_exact_q[lane]),
                .scale_exponent_i (final_scale_q[lane]),
    ...
    assign converter_return_row = convert_meta_row_q[CONVERT_META_DEPTH-1];
    assign converter_return_tag = convert_meta_tag_q[CONVERT_META_DEPTH-1];
    ...
    assign result_tag_o = result_valid_o ? result_queue_meta[24:17] : 8'd0;
    assign result_row_o = result_valid_o ? result_queue_meta[28:25] : 4'd0;
    assign result_pop = result_valid_o && result_ready_i;
    assign busy_o = (!STREAM_METADATA_MODE && command_active) ||
                    partial_stage_valid_q || merge_valid_q ||
    ...
    tile_k_accum_result_bank u_result_meta_bank (
        ...
        .mem_write_i         (result_push),
        .mem_write_addr_i    (result_mem_write_addr),
        .mem_write_data_i    ({3'd0, converter_return_row,
                               converter_return_tag,
                               converter_return_last, convert_invalid}),
        .mem_read_i          (result_mem_read),
        .mem_read_addr_i     (result_mem_read_addr),
        .queue_write_slot_i  (result_queue_write_slot_q),
        .queue_read_slot_i   (result_queue_read_slot_q),
    ...
    always_ff @(posedge clk_i) begin
        for (integer merge_lane = 0; merge_lane < 16;
            ...
            cpa_stage_scale_q[merge_lane] <= merge_scale_q[merge_lane];
            ...
            final_scale_q[merge_lane] <= cpa_stage_scale_q[merge_lane];
            ...
            merge_scale_q[merge_lane] <= partial_stage_scale_q[merge_lane];
        end
        ...
        for (integer stage_lane = 0; stage_lane < 16;
            ...
            partial_stage_exact_q[stage_lane] <=
                partial_exact_i[stage_lane*70 +: 70];
            ...
            partial_stage_scale_q[stage_lane] <= $signed(
                partial_scale_exponent_i[stage_lane*10 +: 10]);
        end
    end
    ...
    always_ff @(posedge clk_i) begin
        if (local_control_flush[1]) begin
            ...
            if (!STREAM_METADATA_MODE && start_i && !start_ready_o) begin
    ...
    always_ff @(posedge clk_i) begin
        if (local_control_flush[2]) begin
            ...
            partial_stage_row_q <= incoming_row;
            partial_stage_tag_q <= stream_tag_i;
    ...
    always_ff @(posedge clk_i) begin
        if (local_control_flush[4]) begin
            ...
            convert_meta_row_q[0] <= final_row_q;
            convert_meta_tag_q[0] <= final_tag_q;
            for (integer meta_stage = 1; meta_stage < CONVERT_META_DEPTH;
                ...
                convert_meta_row_q[meta_stage] <=
                    convert_meta_row_q[meta_stage-1];
                convert_meta_tag_q[meta_stage] <=
                    convert_meta_tag_q[meta_stage-1];
            end
...
`ifndef YOSYS
    initial begin
        ...
        assert (ACCUM_WIDTH >= REQUIRED_ACCUM_WIDTH)
            else $error("tile_fp32_k_accumulator ACCUM_WIDTH must cover 2^20 products");
    end
`endif
