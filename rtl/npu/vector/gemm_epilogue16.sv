...
// 16-lane GEMM后处理流水：可选bias、残差以及ReLU。
...
module gemm_epilogue16 #(
    ...
    localparam int unsigned CONTROL_WIDTH = $bits(vector_pkg::epilogue_control_t);
    localparam int unsigned CONTROL_ACTIVATION_LSB = 3;
    localparam int unsigned CONTROL_RESIDUAL_ENABLE_BIT = 5;
    localparam int unsigned CONTROL_BIAS_ENABLE_BIT = 7;
    localparam int unsigned CONTROL_LANE_MASK_LSB = 17;
