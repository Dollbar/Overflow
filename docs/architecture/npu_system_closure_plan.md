...
| DMA and HBM RTL | ADR-0002, NPU HBM and DMA data-path contracts; NPU-017..022 | Request, response, tag, status telemetry, and integrated beat-boundary leaves `VERIFIED`; mover `HOLD` | Implement address generation, translation boundary, and scratchpad movement without changing frozen fields | Functional-model equivalence plus RTL backpressure/saturation |
...
1. Complete the finite DMA mover and translation boundary against the approved beat contract. The five-lane
   ...
   verified by NPU-017 through NPU-022, including local observation of all four frozen response statuses.
