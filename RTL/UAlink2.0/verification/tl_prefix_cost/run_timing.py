"""Run: python3 verification/tl_prefix_cost/run_timing.py --lib-root PATH --sta PATH.
...
build/verification/tl_prefix_cost. Next run_equivalence.py on these exact netlists.
...
def main():
    ...
    parser.add_argument("--candidate", type=Path, default=ROOT / "rtl/tl/tl_control_partition.v")
    parser.add_argument("--label", default="candidate_timing")
    ...
    sources = [dependency_root / n for n in ("tl_credit_admission.v", "tl_control_decode.v", "tl_control_tenure.v")]
    sources += [ROOT / "scripts" / n for n in ("map_tl_control_partition.tcl", "sta_tl_control_partition.tcl", "credit_abc.constr", "check_sta_report.py")]
    ...
    for width in (8, 16):
        ...
        folder.mkdir()
        script = (ROOT / "scripts/map_tl_control_partition.tcl").read_text()
        script = script.replace("set root [file dirname [file dirname [file normalize [info script]]]]", "set root {" + str(ROOT) + "}")
        script = script.replace("yosys read_verilog [file join $root rtl tl $name.v]", 'if {$name eq "tl_control_partition"} {yosys read_verilog {' + str(stage / "candidate.v") + '}} else {yosys read_verilog [file join $root rtl tl $name.v]}')
        ...
        for corner, library in libraries.items():
            for period in ("0.640", "6.400"):
                ...
                script += "source {" + str(ROOT / "scripts/sta_tl_control_partition.tcl") + "}\n"
