"""Run complete UART TX source complete data induction with retained exact inputs.

Run: python3 verification/formal/run_uart_tx_proof.py [--yosys yosys]
Outputs: build/uart_tx_proof_*/snapshot, proof.log and summary.json.
...
def main():
    ...
    names = ('rtl/dl/dl_uart_tx_source.v', 'verification/formal/uart_tx_properties.v',
             'scripts/prove_uart_tx_source.tcl', 'verification/formal/run_uart_tx_proof.py',
             'config/uart_tx_source_contract.json')
    def sha(path):
        return hashlib.sha256(path.read_bytes()).hexdigest()
    hashes = {name: sha(root/name) for name in names}
    (root/'build').mkdir(exist_ok=True)
    run = Path(tempfile.mkdtemp(prefix='uart_tx_proof_', dir=root/'build'))
    for name in names:
        path = run/'snapshot'/name
        path.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(root/name, path)
    version = subprocess.check_output([args.yosys, '-V'], text=True, timeout=20).strip()
    command = [args.yosys, '-Q', '-T', '-c', 'scripts/prove_uart_tx_source.tcl']
    ...
    reset = content.count('UART_TX_RESET_BASE_PROVED\n') == 1
    induction = content.count('UART_TX_FULL_INDUCTION_PROVED\n') == 1
    completion = content.count('UART_TX_FUNCTIONAL_PROVED groups=5 payload_bits=1024\n') == 1
    ...
    record = {'directory': str(run.relative_to(root)), 'source_sha256': hashes,
              ...
              'scope': 'binary induction over all61 input bits and complete1024-bit held-payload relation; no FIFO, PHY or wall-time liveness proof'}
