"""Snapshot and prove one declared data plane of the actual RX path.
A complete32-bit word claim requires every bit0..31; use the matrix entry for that scope.

Run: python3 verification/formal/run_uart_rx_content_proof.py --kd28-root /authorized/models --depth 128 --proof-bit 0
...
NAMES=('rtl/dl/dl_uart_rx_path.v','rtl/upli/upli_receive_fifo.v','rtl/upli/upli_receive_storage.v',
       'verification/formal/uart_rx_content_properties.v','verification/formal/uart_rx_word_invariant.v',
       'verification/formal/run_uart_rx_content_proof.py','scripts/prove_uart_rx_content.tcl','config/uart_rx_path_contract.json',
       'verification/formal/run_uart_rx_content_matrix.py','scripts/prove_receive_rotation.tcl','verification/formal/receive_rotation_bounds.v')
...
def run_proof(root,dep,depth,content,yosys='yosys',proof_bit=-1):
    ...
    if content is not True or type(proof_bit) is not int or not 0<=proof_bit<=31:raise ValueError('actual content mode and bit0..31 required')
    (root/'build').mkdir(exist_ok=True)
    run=Path(tempfile.mkdtemp(prefix='uart_rx_content_',dir=root/'build'))
    ...
    env=os.environ.copy();env.update(UALINK_DEPTH=str(depth),UALINK_CONTENT=str(int(content)),UALINK_PROOF_BIT=str(proof_bit),
       UALINK_KD28_ROOT=str(dep),UALINK_BUILD_DIR=str(run/'proof'))
    command=[yosys,'-Q','-T','-c','scripts/prove_uart_rx_content.tcl']
    record=dict(directory=str(run.relative_to(root)),depth=depth,content=bool(content),proof_bit=proof_bit,compared_output_bits=124 if proof_bit<0 else 93,
    ...
    record.update(exit_status=r,log_sha256=sha(run/'proof.log'),elapsed_seconds=time.monotonic()-start,
       reset_proved='UART_RX_CONTENT_RESET_BASE_PROVED\n' in text,
       induction_proved='UART_RX_CONTENT_INDUCTION_PROVED\n' in text,
    ...
    record['passed']=(r==0 and record['reset_proved'] and record['induction_proved'] and
       ...
       f'UART_RX_CONTENT_PLANE_PROVED depth={depth} bit={proof_bit} compared_outputs=93 memory_rows={depth} data_bits=1\n' in text and
...
def main():
    ...
    p.add_argument('--depth',type=int,default=128);p.add_argument('--yosys',default='yosys')
    p.add_argument('--proof-bit',type=int,required=True,choices=range(32));a=p.parse_args();record=run_proof(ROOT,a.kd28_root,a.depth,True,a.yosys,a.proof_bit)
