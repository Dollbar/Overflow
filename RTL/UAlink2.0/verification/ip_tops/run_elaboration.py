...
def main():
    ...
    p.add_argument('--label',required=True);p.add_argument('--synth',action='store_true');p.add_argument('--transactions',action='store_true');a=p.parse_args()
    ...
    result=dict(complete=False,endpoint_transaction_mode=int(a.transactions),mode='generic_synthesis' if a.synth else 'elaboration',technology_sta=False,functional_completeness=False,sources={str(f):hashlib.sha256(f.read_bytes()).hexdigest() for f in sources},results=[])
    for top in ('ualink_endpoint_top','ualink_switch_top'):
        ...
        if a.transactions and top=='ualink_endpoint_top':command+='chparam -set TRANSACTION_MODE 1 ualink_endpoint_top\n'
