"""Run the actual UART SRAM/FIFO/source/arbiter path with retained evidence.

Run: python3 verification/rtl/run_uart_tx_path.py --kd28-root /authorized/root
     --depth 128 --seed 17 --cycles 20000 --half-period-ps 320
Outputs: build/uart_tx_path_*/snapshot, independent vectors, logs, summary.json.
Next: inspect accepted-write scoreboard evidence; physical SRAM timing is separate.
...
def main():
    ...
    rtl = ('rtl/upli/upli_receive_fifo.v', 'rtl/upli/upli_receive_storage.v',
           'rtl/dl/dl_uart_tx_source.v', 'rtl/dl/dl_message_arbiter.v', 'rtl/dl/dl_uart_tx_path.v')
    names = rtl+('verification/rtl/uart_tx_path_tb.v', 'verification/rtl/uart_tx_path_vectors.py',
                 'verification/rtl/run_uart_tx_path.py', 'model/ualink/receive_fifo.py',
                 'model/ualink/uart_tx_source.py', 'model/ualink/dl_message_scheduler.py',
                 'model/__init__.py', 'model/ualink/__init__.py', 'config/kd28_verilator.vlt',
                 'config/uart_tx_source_contract.json', 'config/uart_tx_path_contract.json')
    def sha(path):
    ...
    run = Path(tempfile.mkdtemp(prefix='uart_tx_path_', dir=root/'build'))
    ...
    try:
        invoke([sys.executable, 'verification/rtl/uart_tx_path_vectors.py', '--depth', str(args.depth),
        ...
        invoke([args.verilator, '--binary', '--timing', '--language', '1364-2001', '-Wall',
                '--top-module', 'uart_tx_path_tb', '--Mdir', str(run/'obj'),
                f'-GC_HALF_PERIOD_PS={args.half_period_ps}', f'-GC_TX_DEPTH={args.depth}',
                'config/kd28_verilator.vlt', *rtl, 'verification/rtl/uart_tx_path_tb.v',
                *(str(external/name) for name in DEPENDENCIES)], 'compile.log', 180)
        invoke([str(run/'obj/Vuart_tx_path_tb'), f'+VECTORS={run}/vectors.mem'], 'sim.log', 60)
        matches = re.findall(r'^PASS uart_tx_path rows=(\d+) writes=(\d+) payloads=(\d+) uart_messages=(\d+) other_messages=(\d+) resets=(\d+)$',
                             (run/'sim.log').read_text(), re.M)
        assert len(matches) == 1, 'missing or duplicate PASS marker'
        observed = dict(zip(('rows', 'writes', 'payloads', 'uart_messages', 'other_messages', 'resets'),
                            map(int, matches[0])))
        assert all(observed[key] == expected[key] for key in observed), (observed, expected)
        assert expected['served_sources'] == list(range(11)) and expected['max_buffered'] == args.depth
        assert expected['counter_wraps'] >= 1 and expected['full_rejections'] >= 4
        assert expected['rows'] >= args.cycles and expected['payloads'] >= 5000
