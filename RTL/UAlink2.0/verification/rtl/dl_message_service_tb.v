...
module dl_message_service_tb; // 仲裁器测试模块，检查完整 DWORD、来源、边界和 UART 所有权。
    ...
    dl_message_arbiter Arbiter_Inst ( // 实例化真实仲裁器，不接行为替代模型。
        .i_clk(i_clk), .i_uart_cancel(1'b0), .i_rstn(stimulus[370]), .i_segment_available(stimulus[369]), // 同步复位与真实输入服务事件。
