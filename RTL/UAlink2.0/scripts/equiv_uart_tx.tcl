# Run: UALINK_TARGET=source UALINK_LIBERTY=/authorized/cold.lib UALINK_NETLIST=/exact/mapped.v yosys -Q -T -c scripts/equiv_uart_tx.tcl
...
if {$env(UALINK_TARGET) ni {source path}} {error "Unknown UART target"}
foreach key {UALINK_LIBERTY UALINK_NETLIST} {
    if {![file isfile $env($key)]} {error "Missing $key file"}
}
set root [file dirname [file dirname [file normalize [info script]]]]
set top dl_uart_tx_$env(UALINK_TARGET)
set source_prefix ""
set comparisons 21
set inputs 9
if {$env(UALINK_TARGET) eq "path"} {
    ...
    foreach name {upli/upli_receive_fifo.v upli/upli_receive_storage.v dl/dl_message_arbiter.v dl/dl_uart_tx_path.v} {
        yosys read_verilog [file join $root rtl $name]
    }
    yosys chparam -set C_TX_DEPTH $depth $top
    set source_prefix Source_Inst.
}
yosys read_verilog [file join $root rtl dl dl_uart_tx_source.v]
...
foreach name {reg_busy reg_ready reg_payload_count cnt_loaded cnt_word_index cnt_held cnt_tx reg_latest_fc staged_payload} {
...
if {$env(UALINK_TARGET) eq "path"} {
    ...
    foreach name {reg_group_next reg_basic_next reg_control_next reg_uart_next reg_uart_locked cnt_uart_index reg_uart_last} {
        foreach side {gold gate} {
            yosys select -assert-count 1 $side/w:Arbiter_Inst.$name
            yosys expose $side/w:Arbiter_Inst.$name
        }
    }
    ...
    set comparisons [expr {18+9+7+8+8*$count}]
    set inputs [expr {10+$count}]
    if {$count > 1} {
    ...
    puts "UART_TX_MAPPING_MACRO_PORT_SCOPE count=$count shared_q_buses=$count transaction_ports=[expr {8*$count}]"
}
yosys miter -equiv -make_outputs -make_outcmp -flatten gold gate uart_tx_mapping_miter
yosys hierarchy -check -top uart_tx_mapping_miter
yosys opt
yosys wreduce
yosys techmap
yosys opt -full
...
yosys select -assert-count $comparisons uart_tx_mapping_miter/o:cmp_*
yosys select -assert-count $inputs uart_tx_mapping_miter/i:*
yosys select -clear
yosys sat -verify -seq 2 -timeout 120 -set-at 1 in_i_rstn 0 -prove trigger 0 -prove-skip 1 uart_tx_mapping_miter
puts "UART_TX_MAPPING_RESET_PROVED"
yosys sat -verify -seq 2 -timeout 120 -set-at 1 trigger 0 -prove trigger 0 -prove-skip 1 uart_tx_mapping_miter
puts "UART_TX_MAPPING_INDUCTION_PROVED"
puts "UART_TX_MAPPING_PROVED target=$env(UALINK_TARGET) comparisons=$comparisons"
