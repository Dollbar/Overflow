...
foreach change $journal {
    if {[llength $change] != 3} {error "Malformed sender sizing journal entry"}
    ...
    yosys select -assert-count 1 upli_burst_sender/c:$instance
    yosys select -assert-count 1 upli_burst_sender/c:$instance upli_burst_sender/t:$original %i
    yosys chtype -set $replacement upli_burst_sender/c:$instance
}
yosys select *
