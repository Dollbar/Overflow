# Run: UALINK_TARGET=source UALINK_LIBERTY=/authorized/corner.lib UALINK_NETLIST=/exact/mapped.v UALINK_PERIOD_NS=0.640 sta -exit scripts/sta_uart_tx.tcl
...
# Next: strict check_uart_tx_sta_report.py and provenance checks; synthetic macros are not silicon signoff.
if {[catch {
    ...
    link_design $top
    create_clock -name UART_TX -period $env(UALINK_PERIOD_NS) [get_ports i_clk]
    set_clock_uncertainty -setup 0.032 [get_clocks UART_TX]
    set_clock_uncertainty -hold 0.010 [get_clocks UART_TX]
    set_clock_transition 0.050 [get_clocks UART_TX]
    set_input_delay -clock UART_TX -max 0.128 [all_inputs -no_clocks]
    set_input_delay -clock UART_TX -min 0.020 [all_inputs -no_clocks]
    set_input_transition 0.050 [all_inputs -no_clocks]
    set_output_delay -clock UART_TX -max 0.128 [all_outputs]
    set_output_delay -clock UART_TX -min -0.020 [all_outputs]
    set_load 0.005 [all_outputs]
    puts "UART_TX_PROFILE target=$env(UALINK_TARGET) depth=$depth macro_view=$view"
    set path_classes {}
    if {$env(UALINK_TARGET) eq "path"} {
        ...
        puts "UART_TX_LIBRARY name=$memory_library"
        puts "UART_TX_MACRO cell=$expected_cell count=[llength $macros]"
        puts "UART_TX_PINS write_clocks=[llength $wclocks] read_clocks=[llength $rclocks] read_outputs=[llength $macro_outputs] write_data=[llength $macro_d] read_address=[llength $macro_ra] write_address=[llength $macro_wa]"
    ...
    foreach {kind sources sinks} $path_classes {
        ...
        puts "UART_TX_PATH $kind min_slack_ns=$minimum_slack max_slack_ns=$maximum_slack"
    }
    ...
    puts stderr "FAIL UART TX STA: $reason"
    exit 1
}
exit 0
