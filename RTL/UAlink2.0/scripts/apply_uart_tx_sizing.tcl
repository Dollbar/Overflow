...
foreach change $journal {
    if {[llength $change] != 3} {error "Malformed UART source sizing journal entry"}
    ...
    yosys select -assert-count 1 dl_uart_tx_source/c:$instance
    yosys select -assert-count 1 dl_uart_tx_source/c:$instance dl_uart_tx_source/t:$original %i
    yosys chtype -set $replacement dl_uart_tx_source/c:$instance
}
yosys select *
