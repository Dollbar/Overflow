...
脚本`materialize_ip_scaffold.py`仅创建不存在的文件；已存在文件若与预期不同则拒绝，
不会覆盖未来实现。`--check`只读核对全部预期文件，不创建目录或修改文件。
结构检查器独立核查模块声明、角色实例与slot，并使用Yosys展开角色聚合层核对
实际常量位图；源码文件存在与功能已实现分别报告。

## 生成位置与稳定角色位图

本轮库存为202个RTL模块：57个`existing_partial`保持原有实现，145个`planned`
...
生成器不会覆盖不同的旧聚合文件；库存调整导致预期聚合变更时也需要显式审查修改，
防止自动生成流程删除已经接入的实现。结构通过不代表规范功能齐全、时序达标、
安全协议完成或整IP交付完成。
