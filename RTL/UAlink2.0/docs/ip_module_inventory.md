...
快照记录 57 个实际存在RTL源、145 个计划补齐RTL模块、13 个软件管理模块和4类外部平台边界。已有顶层文件也只是 `existing_partial`；文件存在不代表其所有功能已实现。骨架生成后，库存的 `planned` 仍表示功能未实现，不能因文件出现就自动变更为完成。
...
Endpoint使用128-bit角色内位图，已分配slot0–108（109个）；Switch使用128-bit角色内位图，已分配slot0–126（127个）。每个planned条目自带 `feature_slots`。shared模块通过同一稳定module id映射到各角色独立slot。未分配位为0，已分配而未实现位为1；现有部分RTL没有壳slot并不代表完整符合性。

不得因排序或实现进展重编号已分配slot；实现完成时保留slot，仅更新实现状态与证据。新增模块追加slot；超过128时先显式扩展位图版本/本地接口，不截断。此位图是研发可观测性，不是标准能力协商字段。
...
| `rtl/endpoint/endpoint_read_encode.v` | E / planned / E:11 | 已冻结单64-byte普通Read的128-bit字段编码 |
| `rtl/endpoint/endpoint_response_encode.v` | E / planned / E:12 | 已冻结单Beat普通Read Response的64-bit字段编码 |
...
| `rtl/switch/switch_route_lookup.v` | S / planned / S:53 | 按目的ID/角色选择合法出口或INC处理，不截断ID |
