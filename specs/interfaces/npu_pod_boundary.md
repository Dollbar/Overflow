...
| Decoded internal command | external KD-ISA frontend | NPU decoded-command sink | `BASELINED / VERIFIED` | ADR-0007 and `npu_decoded_command.md` freeze the versioned 372-bit Task/local/DMA envelope; source-side KD-ISA decoding remains external |
...
| Internal DMA descriptor | NPU-local scheduler/adapter | DMA frontend | `BASELINED / VERIFIED` | `npu_dma_command.md` freezes the internal linear/strided command used by the mover; KD-ISA packing, IOVA, protection, chaining, and cancellation remain external or held |
...
| Pod-shared SRAM DMA client request | DMA | shared SRAM | `BASELINED / VERIFIED` | `npu_pod_shared_sram.md` freezes sixteen DMA clients, eight banks, full-beat writes, independent round-robin read/write arbitration, and ready-valid backpressure; compute/NoC clients and ECC remain held |
| Pod shared-to-private transfer | Pod loader | compute-local SRAM writes | `BASELINED / VERIFIED LEAF` | `npu_pod_local_transfer.md` freezes paired full-beat reads and one-to-eight 128-bit Tensor/Vector writes; wider production loading remains a later revision |
| Complete two-cluster Pod | task/DMA/local-transfer producers | compute/result/HBM consumers | `BASELINED / VERIFIED` | `npu_compute_pod.md` freezes the single-clock composition, task/status matching, DMA/loader SRAM sharing, result flattening, and lossless quiesce boundary |
| Pod/NoC logical attachment | pod packet clients | pod router | `BASELINED / VERIFIED LEAF` | ADR-0005 and `npu_pod_noc_attachment.md` freeze ready-valid, one 128-bit control lane, two 1,024-bit data lanes, endpoint metadata, quiesce, and diagnostics |
| Fixed 2 by 4 Pod array | per-Pod command/HBM/packet lanes | NPU integration | `BASELINED / VERIFIED LEAF` | `npu_2x4_pod_array.md` freezes eight managed Pods, same-ID HBM affinity, per-Pod clock/reset inputs, and eight router-independent attachments; it contains no router or CDC behavior |
| NoC router/credit/VC implementation | pod attachment | inter-pod mesh | `BASELINED / VERIFIED LOGIC` | ADR-0008 and the NoC Router/CDC contracts freeze VC mapping, credits, deterministic routing, arbitration, telemetry, reset, and CDC; physical 2 GHz/CDC signoff remains held |
| Integrated 2 by 4 Pod/NoC system | Pod packet clients | remote Pod packet clients | `BASELINED / VERIFIED LOGIC` | `npu_2x4_pod_noc_system.md` joins all eight attachments, CDC pairs, and Mesh routers while preserving opaque payloads and independently synchronized clocks/reset/quiesce |
...
The admitted Pod/NoC attachment is synchronous to its supplied Pod clock. The multi-clock system shell owns
the paired asynchronous FIFOs, independent reset release, synchronized quiesce, and NoC-domain status
aggregation defined by the NoC CDC and system v0.1 contracts. Any command, HBM, KDLink, or management
clock crossing outside that closed path remains `HOLD`. Before such RTL is admitted,
...
- the versioned 2 by 4 VC/credit Mesh, per-Pod CDC, and joint Pod/NoC transport shell with opaque payloads.
...
- packet-client adapters that assign remote DMA/SRAM, collective, retry, timeout, or software-completion
  semantics to the otherwise opaque Pod/NoC payload.
